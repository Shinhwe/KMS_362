...
public class PlayersHandler
{
    ...
    if (GameConstants.isYeti(chr.getJob()) || GameConstants.isPinkBean(chr.getJob()))
    {
      ...
      switch (chr.getTouchedRune())
      {
        ...
        case 5:
          ...
          if (!spawn)
          {
            List<MapleMapObject> monstersInRange = chr.getMap().getMapObjectsInRange(chr.getPosition(), 2.0E7D, Collections.singletonList(MapleMapObjectType.MONSTER));
            if (!monstersInRange.isEmpty())
            {
              int random = Randomizer.rand(0, monstersInRange.size() - 1);
              int mobid = monstersInRange.get(random).getObjectId();
              ...
              if (monster != null && !monster.getStats().isBoss())
              {
                for (int i = 0; i < 3; i++)
                {
                  chr.getMap().makeEliteMonster(monster.getId(), Randomizer.rand(0, 2), (chr.getMap()).monsterSpawn.get(Randomizer.rand(0, (chr.getMap()).monsterSpawn.size() - 1)).getPosition(), true, false);
                }
              }
            }
          }
  ...
  public static void ColorCardHandler (LittleEndianAccessor slea, MapleClient c)
  {
    ...
    {
      if (chr.getColorCardInstance().getCardList().get(0).intValue() == type || type == 3)
      {
        ...
        switch (chr.getColorCardInstance().getCardList().get(0).intValue())
        {
          case 0:
  ...
  public static void MapleTyoonKitchenSuc (LittleEndianAccessor slea, MapleClient c)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          if (list.getRight().intValue() == 0)
          {
            if (list.getLeft().intValue() == c.getPlayer().getMtk().getItemid())
            {
