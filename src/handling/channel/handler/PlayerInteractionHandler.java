  ...
  public static final void PlayerInteraction(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr)
  {
    ...
    switch (action)
    {
      ...
      case REMOVE_ITEM:
      {
        ...
        c.getSession().writeAndFlush(PlayerShopPacket.shopItemUpdate(shop5));
        break;
      }
      ...
      case SELECT_CARD:
      {
        ...
        if (turn == 1)
        {
          ...
          {
            game2.getMCOwner().getClient().getSession().writeAndFlush(PlayerShopPacket.getMatchCardSelect(turn, slot3, fs, turn));
          }
          game2.setTurn(0);
          return;
        }
        ...
        {
          game2.broadcastToVisitors(PlayerShopPacket.getMatchCardSelect(turn, slot3, fs, game2.isOwner(chr) ? 0 : 1));
          game2.nextLoser();
        }
        game2.setTurn(1);
        ...
        switch (type)
        {
          case 0:
          {
            ...
            if (nextPlayer != null)
            {
              ...
              if (oc.getLastCard().getType() == 8)
              {// || (oc.getLastCard().getType() == 12 && oc.getLastCard().getType() == 3)) {
      ...
      default:
      {
        System.out.println("Unhandled interaction action by " + chr.getName() + " : " + action + ", " + slea);
        break;
      }
    }
  }
