...
public class PlayerHandler
{
  ...
  public static final void TakeDamage (LittleEndianAccessor slea, MapleClient c, MapleCharacter chr)
  {
    ...
    if (Randomizer.isSuccess(50) && GameConstants.isXenon(chr.getJob()) && chr.getBuffedValue(SecondaryStat.AegisSystem) != null && chr.getSkillCustomValue(36110004) == null)
    {
      ...
      if (chr.getSummon(400041044) != null && (box = new Rectangle(chr.getSummon(400041044).getTruePosition().x - 320, chr.getSummon(400041044).getTruePosition().y - 490, 640, 530)).contains(chr.getTruePosition()))
      {
        for (int i = 0; i < 5; ++i)
        {
    ...
    if (type > -2)
    {
      ...
      if (damage > 0 && chr.getSkillLevel(5120011) > 0)
      {
        ...
        {
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 5120011, 4, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getPosition(), null, null));
      ...
      if (chr.getBuffedEffect(SecondaryStat.BodyOfSteal) != null && chr.bodyOfSteal < (bodyOfSteal = chr.getBuffedEffect(SecondaryStat.BodyOfSteal)).getY())
      {
        ...
        chr.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, bodyOfSteal, chr));
      }
      ...
      {
        damage = 0;
        chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 151121004, 10, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getPosition(), null, null));
    ...
    if ((chr.getJob() == 2711 || chr.getJob() == 2712) && chr.getSkillLevel(27110007) > 0)
    {
      ...
      if (chr.getStat().getHp() / chr.getStat().getCurrentMaxHp() * 100L < chr.getStat().getMp() / chr.getStat().getCurrentMaxMp(chr) * 100L)
      {
        ...
        c.getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, skill.getEffect(critical), chr));
      }
    ...
    if (damage > 0)
    {
        ...
        if (attacker != null)
        {
          ...
          for (MapleSummon sum : chr.getMap().getAllSummonsThreadsafe())
          {
            ...
            if (sum.getPosition().x - 370 < chr.getPosition().x && sum.getPosition().x + 370 > chr.getPosition().x && sum.getPosition().y - 235 < chr.getPosition().y && sum.getPosition().y + 235 > chr.getPosition().y)
            {
              ...
              {
                if (attacker.getStats().getHp() / 2L > (damage * 13L))
                {
                  damaged = true;
                }
              }
              else if (chr.getParty() != null && owner.getParty() != null && chr.getParty().getId() == owner.getParty().getId() && owner.getMapId() == chr.getMapId() && attacker.getStats().getHp() / 2L > (damage * 13L))
              {
                damaged = true;
              }
            }
      ...
      if (chr.getBuffedValue(400011011))
      {
        ...
        if (buffowner.getRhoAias() > 1)
        {
          ...
          buffowner.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, eff, buffowner));
      ...
      {
        chr.addMP(-40L);
        chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 162001005, 10, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getTruePosition(), null, null));
      ...
      if (type == -1 || type == 0)
      {
        ...
        if (chr.getBuffedValue(152000009) && chr.blessMark > 0)
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 152100011, 10, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getPosition(), null, null));
  ...
  public static final void BossWarp (LittleEndianAccessor slea, MapleCharacter chr)
  {
    ...
    if (chr.getParty() != null)
    {
      ...
      if (size == chr.getParty().getMembers().size())
      {
        for (MaplePartyCharacter chrz : chr.getParty().getMembers())
        {
          ...
          curChar.getPlayer().setKeyValue(210406, "Return_BossMap", String.valueOf(curChar.getMapId()));
    ...
    {
      chr.getPlayer().setKeyValue(210406, "Return_BossMap", String.valueOf(chr.getMapId()));
  ...
  public static final void CancelBuffHandler (LittleEndianAccessor slea, MapleCharacter chr)
  {
    ...
    {
      int time = (int) (chr.getBuffLimit(1221054) / (effect.getY() * 1000L));
    ...
    if (skillid == 3101009)
    {
      if (chr.getBuffedValue(3101009))
      {
        ...
        if (chr.getRestArrow()[quiver - 1] > 0)
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 3101009, 57, chr.getQuiverType() - 1, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getPosition(), null, null));
        ...
        else
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 3101009, 57, type, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getPosition(), null, null));
    ...
    Label_14276:
    {
      switch (skillid)
      {
        ...
        case 162121042:
        {
          ...
          atoms.add(new SecondAtom(20, c.getPlayer().getId(), 0, 0, 162101000, 0, 0, 0, pos, Collections.singletonList(8)));
        ...
        case 400021123:
        {
          ...
          skills.add(new RangeAttack(400021128, ret.plusPosition2, (ret.rlType > 0) ? 1 : 0, 5850, 8));
          skills.add(new RangeAttack(400021127, ret.plusPosition2, (ret.rlType > 0) ? 1 : 0, 3150, 4));
          skills.add(new RangeAttack(400021126, ret.plusPosition2, (ret.rlType > 0) ? 1 : 0, 2250, 4));
          skills.add(new RangeAttack(400021125, ret.plusPosition2, (ret.rlType > 0) ? 1 : 0, 1560, 4));
          skills.add(new RangeAttack(400021124, ret.plusPosition2, (ret.rlType > 0) ? 1 : 0, 630, 4));
        ...
        case 162121010:
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 162121010, 1, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, ret.plusPosition2, null, null));
        ...
        case 400021122:
        {
          ...
          for (int j = 0; j < 7 + ((skillid == 400021122) ? 0 : size); ++j)
          {
            ...
            atoms2.add(new SecondAtom((ret.subAttackType == 1) ? 22 : ((ret.subAttackType == 2) ? 25 : ((ret.subAttackType == 4) ? 26 : ((ret.subAttackType == 8) ? 27 : 0))), c.getPlayer().getId(), mobid2, 1140, skillid, 4000, 0, 1, new Point(ret.plusPosition2.x + Randomizer.rand(-200, 200), ret.plusPosition2.y + Randomizer.rand(-350, -50)), Collections.emptyList()));
        ...
        case 162111002:
        {
          ...
          if (chr.getMap().getSecondAtom(chr.getId(), skillid) != null)
          {
            ...
            atoms2.add(new SecondAtom(32, c.getPlayer().getId(), 0, 0, skillid, 90000, 0, 1, ret.plusPosition2, Collections.singletonList(chr.getMap().getSecondAtom(chr.getId(), skillid).getObjectId())));
            effect.applyTo(chr);
            chr.getMap().spawnSecondAtom(chr, atoms2, 0);
            break;
          }
          atoms2.add(new SecondAtom(31, c.getPlayer().getId(), 0, 0, skillid, 10000, 0, 1, ret.plusPosition2, Collections.singletonList(0)));
          chr.getMap().spawnSecondAtom(chr, atoms2, 0);
          c.getSession().writeAndFlush(CField.skillCooldown(skillid, linkEffect.getCooldown(chr)));
          chr.addCooldown(skillid, System.currentTimeMillis(), linkEffect.getCooldown(chr));
          break;
        ...
        case 400001011:
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(400001011, mobList, true, 0));
          ...
          if (chr.getMapId() != 450013700 && chr.getParty() != null)
          {
            for (final MaplePartyCharacter chr3 : chr.getParty().getMembers())
            {
              if (chr3.isOnline() && chr3.getMapid() == chr.getMapId())
              {
                ...
                chr5.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr5, 0, skillid, 4, 0, 0, (byte) ((chr5.getTruePosition().x > pos.x) ? 1 : 0), true, chr5.getPosition(), null, null));
        ...
        case 2301002:
        {
          ...
          if (chr.getMapId() != 450013700 && chr.getParty() != null)
          {
            for (final MaplePartyCharacter chr2 : chr.getParty().getMembers())
            {
              if (chr2.isOnline() && chr.getId() != chr2.getPlayer().getId() && chr2.getMapid() == chr.getMapId())
              {
                ...
                chr6.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr6, 0, skillid, 4, 0, 0, (byte) ((chr6.getTruePosition().x > pos.x) ? 1 : 0), true, chr6.getPosition(), null, null));
          ...
          c.getSession().writeAndFlush(CField.skillCooldown(skillid, cool));
        ...
        case 4211006:
        {
          ...
          final List<MapleMapObject> drops = c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getPosition(), 500000.0, Collections.singletonList(MapleMapObjectType.ITEM));
        ...
        case 22170070:
        {
          ...
          final List<MapleMapObject> mobjects = c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getPosition(), 600000.0, Collections.singletonList(MapleMapObjectType.MONSTER));
        ...
        case 30001061:
        {
          ...
          if (mob4 != null)
          {
            ...
            c.getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, skillid, 1, 0, 0, (byte) (success ? 1 : 0), true, null, null, null));
        ...
        case 33121255:
        {
          ...
          {
            final List<MapleMapObject> objs = chr.getMap().getMapObjectsInRange(chr.getTruePosition(), 500000.0, Collections.singletonList(MapleMapObjectType.MONSTER));
        ...
        case 152120001:
        {
          ...
          effect.applyTo(chr);
          c.getSession().writeAndFlush(CWvsContext.enableActions(chr, true, false));
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, skillid, 1, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, pos, null, null));
        ...
        case 400021088:
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, skillid, 10, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getTruePosition(), null, null, ret));
          break;
        }
        case 400021099:
        {
        ...
        default:
        {
          if (skillid == 64001012)
          {
            ...
            chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, oldskillid, skillid, 1, 0, 0, direction, true, pos, null, null));
    ...
    if (attack.skill != 0)
    {
      ...
      if (GameConstants.isKadena(chr.getJob()))
      {
        ...
        else if (attack.skill == 64121001)
        {
          ...
          {
            if (info.left.intValue() != 0 && !chr.weaponChanges1.containsKey(info.left) && chr.weaponChanges1.size() < 8)
            {
              chr.weaponChanges1.put(info.left, info.right);
            }
          }
      ...
      if (GameConstants.isPathFinder(chr.getJob()))
      {
        ...
        if (attack.skill == 3321005 && attack.targets > 0 && (c.getPlayer()).문양 == 1)
        {
          ...
          List<MapleMapObject> objs = c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getTruePosition(), 400000.0D, Collections.singletonList(MapleMapObjectType.MONSTER));
          List<Integer> monsters = new ArrayList<>();
      ...
      else if (GameConstants.isEunWol(chr.getJob()))
      {
        if (chr.getBuffedValue(25101009) && SkillFactory.getSkill(25100009).getSkillList().contains(Integer.valueOf(attack.skill)) && attack.targets > 0)
        {
          ...
          if (attack.skill == 400051079)
          {
            ...
            List<MapleMapObject> objs = chr.getMap().getMapObjectsInRange(chr.getTruePosition(), 500000.0D, Collections.singletonList(MapleMapObjectType.MONSTER));
            List<Integer> targets = new ArrayList<>();
            for (int i = 0; i < ((attack.skill == 400051079) ? (effect.getV2() / 3) : 1); i++)
            {
              targets.add(Integer.valueOf((objs.size() > i) ? objs.get(i).getObjectId() : 0));
  ...
  public static final void rangedAttack (LittleEndianAccessor slea, MapleClient c, MapleCharacter chr)
  {
    ...
    if (attack.skill == 4121017)
    {
      ...
      {
        final List<SecondAtom> atoms = new ArrayList<SecondAtom>();
        atoms.add(new SecondAtom(0x21, c.getPlayer().getId(), 0, 4121020, 3000, 0, 1, new Point(attack.attackPosition.x, attack.attackPosition.y - 88), Collections.singletonList(0)));
        atoms.add(new SecondAtom(0x21, c.getPlayer().getId(), 0, 4121020, 3000, 0, 1, new Point(attack.attackPosition.x, attack.attackPosition.y + 88), Collections.singletonList(0)));
        atoms.add(new SecondAtom(0x21, c.getPlayer().getId(), 0, 4121020, 3000, 0, 1, new Point(attack.attackPosition.x + 80, attack.attackPosition.y + 43), Collections.singletonList(0)));
        atoms.add(new SecondAtom(0x21, c.getPlayer().getId(), 0, 4121020, 3000, 0, 1, new Point(attack.attackPosition.x - 90, attack.attackPosition.y + 43), Collections.singletonList(0)));
        atoms.add(new SecondAtom(0x21, c.getPlayer().getId(), 0, 4121020, 3000, 0, 1, new Point(attack.attackPosition.x + 95, attack.attackPosition.y - 57), Collections.singletonList(0)));
        atoms.add(new SecondAtom(0x21, c.getPlayer().getId(), 0, 4121020, 3000, 0, 1, new Point(attack.attackPosition.x - 95, attack.attackPosition.y - 57), Collections.singletonList(0)));
        c.getPlayer().spawnSecondAtom(atoms);
    ...
    if (chr.getBuffedValue(SecondaryStat.閃光幻象) != null && attack.skill != 3111016 && attack.skill != 3100001)
    {
      ...
      if (!Arrays.asList(skills).contains(attack.skill))
      {
        ...
        else if (chr.플레시미라주스택 == count)
        {
          ...
          {
            List<MapleMapObject> objs = chr.getMap().getMapObjectsInRange(chr.getTruePosition(), 500000.0, Collections.singletonList(MapleMapObjectType.MONSTER));
            if (objs.size() > 0)
            {
              final List<SecondAtom> atoms = new ArrayList<SecondAtom>();
              Point pos = new Point(attack.attackPosition.x + Randomizer.rand(-250, 250), attack.attackPosition.y + Randomizer.rand(-250, 250));
              atoms.add(new SecondAtom(0x27, chr.getId(), 0, 3111016, 3000, 0, 6, pos, Collections.singletonList(0)));
              chr.spawnSecondAtom(atoms);
              chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 3111015, 83, 0, 0, (byte) 0, true, pos, null, null));
    ...
    if (!AOE && chr.getBuffedValue(SecondaryStat.SoulArrow) == null && !noBullet)
    {
      ...
      if (attack.csstar > 0)
      {
        ...
        {
          chr.addKV("csstar", String.valueOf(visProjectile));
        }
    ...
    if (portal != null)
    {
      ...
      else if (portal.getScriptName().contains("Blooming") && c.getPlayer().getMapId() == 993192600)
      {
        ...
        else if (portal.getName().contains("goal00"))
        {
          ...
          c.getPlayer().addKV("BloomingRaceRank", String.valueOf(BloomingRace.getRank()));
    ...
    switch (status)
    {
      case 0:
      {
        ...
        if (skinsize < skinroom)
        {
          ...
          {
            if (chr.getKeyValue(13191, String.valueOf(i)) == GameConstants.getDSkinNum((int) chr.getKeyValue(7293, "damage_skin")))
            {
              isalready = true;
            }
          }
          if (!isalready)
          {
            chr.setKeyValue(13191, "skins", String.valueOf(skinsize + 1));
            chr.setKeyValue(13191, String.valueOf(skinsize), String.valueOf(GameConstants.getDSkinNum((int) chr.getKeyValue(7293, "damage_skin"))));
          }
        }
        break;
      }
      ...
      case 2:
      {
        ...
        if (status == 1)
        {
          ...
          for (int i = 0; i < skinsize; i++)
          {
            if (chr.getKeyValue(13191, String.valueOf(i)) == skinid)
            {
              finded = true;
            }
            if (finded)
            {
              if (chr.getKeyValue(13191, String.valueOf(i + 1)) != -1)
              {
                chr.setKeyValue(13191, String.valueOf(i), String.valueOf(chr.getKeyValue(13191, String.valueOf(i + 1))));
                if ((i + 1) == skinsize || chr.getKeyValue(13191, String.valueOf(i + 2)) == -1)
                {
                  chr.removeKeyValue(13191, String.valueOf(i + 1));
                }
              }
            }
          }
  ...
  public static final void ChangeInner (LittleEndianAccessor slea, MapleClient c)
  {
    ...
    int a = (c.getPlayer().getInnerSkills().get(0).getRank() == 0) ? 100 : ((c.getPlayer().getInnerSkills().get(0).getRank() == 1) ? 200 : ((c.getPlayer().getInnerSkills().get(0).getRank() == 2) ? 1500 : 8000));
    int plus = (c.getPlayer().getInnerSkills().get(0).getRank() == 2) ? 1500 : 3000;
    int plus2 = (c.getPlayer().getInnerSkills().get(0).getRank() == 2) ? 4000 : 8000;
      ...
      else if (ivholder == null)
      {
        ...
        while (!breakout)
        {
          if (count != 0)
          {
            ...
            if (ivholder.getSkillId() == c.getPlayer().getInnerSkills().get(line - 1).getSkillId() || ivholder.getSkillId() == c.getPlayer().getInnerSkills().get(line2 - 1).getSkillId())
            {
              ivholder = InnerAbillity.getInstance().renewSkill(nowrank, false);
              continue;
            }
    ...
    for (int i = 0; i < wsize; ++i)
    {
      ...
      switch (skillId)
      {
        ...
        case 400021069:
        {
          ...
          c.getPlayer().getBuffedEffect(skillId).applyTo(c.getPlayer(), false, (int) (duration += attackCount));
          return;
        }
        case 152001001:
        case 152110004:
        ...
        case 152120008:
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 152120008, 4, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getTruePosition(), null, null));
      ...
      switch (skillId)
      {
        ...
        case 14000028:
        {
          List<MapleMapObject> objs = c.getPlayer().getMap().getMapObjectsInRange(new Point(x, y), 200000.0, Collections.singletonList(MapleMapObjectType.MONSTER));
          if (objs.size() <= 0)
          {
            continue block23;
          }
  ...
  public static void FieldAttackObjAttack (LittleEndianAccessor slea, MapleCharacter chr)
  {
    ...
    if (type == 0)
    {
      ...
      chr.getMap().broadcastMessage(CField.B2BodyResult(chr, cid, type, type2, key, pos, oldPos, unk1, sourceid, level, duration, unk2, chr.isFacingLeft(), 0, 0, null));
  ...
  public static void FieldAttackObjAction (LittleEndianAccessor slea, MapleCharacter chr)
  {
    ...
    for (MapleMapObject obj : chr.getMap().getMapObjectsInRange(chr.getTruePosition(), Double.POSITIVE_INFINITY, Collections.singletonList(MapleMapObjectType.FIELD)))
    {
    ...
    if (!shot)
    {
      ...
      while (firstsize > 0)
      {
        ...
        if (firstsize > 0)
        {
          ...
          grab.get(Integer.valueOf(i)).add(new PsychicGrabEntry(firstsize, a, -1 - a + i, mobid, secondsize, mobhp, mobmaxhp, unk, rect, unk2));
        }
        i++;
      }
      ...
      for (int i = 80000066; i <= 80000070; i++)
      {
        if (c.getPlayer().getTotalSkillLevel(i) > 0)
        {
          for (Triple<Skill, SkillEntry, Integer> a : c.getPlayer().getLinkSkills())
          {
            if (a.left.getId() == i)
            {
              skills.put(Integer.valueOf(i), a.right);
            }
          }
    ...
    else if (skillid == 80000329)
    {
      ...
      if (c.getPlayer().getTotalSkillLevel(80000378) > 0)
      {
        ...
        for (Triple<Skill, SkillEntry, Integer> a : c.getPlayer().getLinkSkills())
        {
          ...
          {
            skills.put(Integer.valueOf(80000378), a.right);
          }
        }
    ...
    else if (skillid == 80002774)
    {
      ...
      if (c.getPlayer().getTotalSkillLevel(80000000) > 0)
      {
        ...
        for (Triple<Skill, SkillEntry, Integer> a : c.getPlayer().getLinkSkills())
        {
          ...
          {
            skills.put(Integer.valueOf(80000000), a.right);
          }
        }
  ...
  public static void subSkillEffect (LittleEndianAccessor slea, MapleCharacter chr)
  {
    ...
    if (skillId == 400031003 || skillId == 400031004)
    {
      ...
      {
        chr.setSkillCustomInfo(400031333, 0L, (yoyo.getX() * 1000L));
      }
  ...
  public static void HandlePlatformerEnter (LittleEndianAccessor slea, MapleClient c)
  {
    ...
    c.getPlayer().setKeyValue(18838, "count", String.valueOf(c.getPlayer().getKeyValue(18838, "count") - 1L));
    switch (Stage)
    {
      case 1:
  ...
  public static void goldCompleteByPass (MapleClient c)
  {
    try
    {
      ...
      {
        if ((value == 1) ? (item.left.intValue() == k) : (item.left.intValue() == k || item.left.intValue() == k - 1))
        {
          Item addItem;
          if (!MapleInventoryManipulator.checkSpace(c, item.mid.intValue(), item.right.intValue(), ""))
          {
            c.getPlayer().dropMessage(1, "보상을 받기 위한 인벤토리의 공간이 부족합니다.");
            c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
            return;
          }
          if (item.mid.intValue() == 4310291)
          {
            c.getPlayer().dropMessage(1, "보상이 지급되었습니다.");
            c.getPlayer().AddStarDustCoin(1, item.right.intValue());
            break;
          }
          ...
          {
            int quantity = item.right.intValue();
            addItem = new Item(item.mid.intValue(), (short) 0, (short) quantity);
          }
          if (addItem != null)
          {
            MapleInventoryManipulator.addbyItem(c, addItem);
  ...
  public static void warpGuildMap (LittleEndianAccessor slea, MapleCharacter player)
  {
    ...
    switch (id)
    {
      ...
      case 7860:
        ...
        player.setKeyValue(id, "returnMap", String.valueOf(player.getMapId()));
  ...
  public static void Revenant (LittleEndianAccessor slea, MapleClient c)
  {
    ...
    if (savedamage > 0)
    {
      chra.setSkillCustomInfo(400011112, (savedamage > 0) ? (savedamage - (long) savedamage / 100 * effect.getQ2()) : 0L, 0L);
      Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
  ...
  public static void CommandLockAction2 (LittleEndianAccessor slea, MapleClient c)
  {
    ...
    switch (skillid)
    {
      ...
      case 5100015:
        if (c.getPlayer().getKeyValue(1544, String.valueOf(skillid)) != 1L)
        {
          c.getPlayer().setKeyValue(1544, String.valueOf(skillid), "1");
        }
        else
        {
          c.getPlayer().setKeyValue(1544, String.valueOf(skillid), "0");
        }
    ...
    else
    {
      ...
      {
        if (((Pair) list.get(j)).getLeft().equals(Byte.valueOf(Byte.parseByte(id))))
        {
  ...
  public static void Vmatrixstackbuff (MapleClient c, boolean use, LittleEndianAccessor slea, int count1)
  {
    ...
    else if (c.getPlayer().getJob() == 1312)
    {
      ...
      else if (!use && count < 2L)
      {
        ...
        c.getPlayer().setSkillCustomInfo(400031333, 0L, yoyo.getX() * 1000L);
      }
      count = c.getPlayer().getSkillCustomValue(400031003);
  ...
  public static void LiftBreak (MapleCharacter player, LittleEndianAccessor slea)
  {
    ...
    {
      player.cancelEffect(player.getBuffedEffect(64120006), null, true);
      ...
      if ((skillid == 142111006 || skillid == 142120003) && monster != null)
      {
        ...
        if (monster.getBuff(skillid) == null)
        {
          ...
          for (Triple<MonsterStatus, MonsterStatusEffect, Long> status : statusz)
          {
            ...
            {
              status.mid.setValue(status.right.longValue());
    ...
    switch (slea.readInt())
    {
      ...
      case 4:
        ...
        if (give)
        {
          ...
          if (list1.get(get - 1).getPlayerid() > 0)
          {
            if (list1.get(get - 1).getPlayerid() != c.getPlayer().getId())
            {
              c.getPlayer().dropMessage(1, "해당 아이템은 <" + list1.get(get - 1).getName() + "> 캐릭터로 수령이 가능 합니다.");
              c.send(CWvsContext.enableActions(c.getPlayer()));
              return;
            }
            if (list1.get(get - 1).getItemid() / 1000000 != 1)
            {
              c.getPlayer().gainItem(list1.get(get - 1).getItemid(), (short) list1.get(get - 1).getCount());
            }
            else
            {
          ...
          c.send(CWvsContext.InfoPacket.getShowItemGain(list1.get(get - 1).getItemid(), (short) list1.get(get - 1).getCount(), true));
          for (MapleCabinet change : c.getCabiNet())
          {
            if (change.getItemid() == list1.get(get - 1).getItemid() && change.getDelete() == 0)
            {
              change.setDelete(1);
              break;
            }
  ...
  public static void applySpecialCoreSkills (MapleCharacter chr)
  {
    ...
    int coreId = coredata.left.intValue();
    int skillId = coredata.right.intValue();
