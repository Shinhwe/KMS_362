...
public class NPCHandler
{
  ...
  public static final void repairAll(MapleClient c)
  {
      ...
      for (Item item : c.getPlayer().getInventory(type).newList())
      {
        if (item instanceof Equip)
        {
          ...
          if (eq.getDurability() >= 0)
          {
            ...
            if (eqStats.containsKey("durability") && eqStats.get("durability").intValue() > 0 && eq.getDurability() < eqStats.get("durability").intValue())
            {
              double rPercentage = 100.0D - Math.ceil(eq.getDurability() * 1000.0D / eqStats.get("durability").intValue() * 10.0D);
    ...
    for (Map.Entry<Equip, Integer> eqqz : eqs.entrySet())
    {
      ...
      ez.setDurability(eqqz.getValue().intValue());
  ...
  public static final void repair(LittleEndianAccessor slea, MapleClient c)
  {
    ...
    if (eq.getDurability() < 0 || !eqStats.containsKey("durability") || eqStats.get("durability").intValue() <= 0 || eq.getDurability() >= eqStats.get("durability").intValue())
    {
      return;
    }
    double rPercentage = 100.0D - Math.ceil(eq.getDurability() * 1000.0D / eqStats.get("durability").intValue() * 10.0D);
    ...
    eq.setDurability(eqStats.get("durability").intValue());
  ...
  public static final void UseItemQuest(LittleEndianAccessor slea, MapleClient c)
  {
    ...
    for (Item i : c.getPlayer().getInventory(MapleInventoryType.ETC))
    {
      if (i.getItemId() / 10000 == 422)
      {
        ...
        if (questItemInfo != null && questItemInfo.getLeft().intValue() == qid && questItemInfo.getRight() != null && questItemInfo.getRight().contains(Integer.valueOf(itemId)))
        {
          found = true;
          break;
        }
