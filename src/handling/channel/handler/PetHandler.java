...
public class PetHandler
{
  public static void SpawnPet(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr)
  {
    ...
    if (pet != null)
    {
      ...
      else
      {
        ...
        c.getSession().writeAndFlush(CWvsContext.InventoryPacket.clearInventoryItem(MapleInventoryType.CASH, slot, false));
      }
    }
    c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
  }
  ...
  public static final void MovePet(final LittleEndianAccessor slea, final MapleCharacter chr)
  {
    ...
    if (res != null && chr != null && res.size() != 0 && chr.getMap() != null)
    {
      ...
      {
        MapleMapItem mapleMapItem = dropsIterator.next();
        if (mapleMapItem.getMeso() > 0)
        {
          chr.gainMeso(mapleMapItem.getMeso(), true, true, true, true);
          mapleMapItem.setPickedUp(true);
          chr.getMap().broadcastMessage(CField.removeItemFromMap(mapleMapItem.getObjectId(), 5, chr.getId(), 0));
          chr.getMap().removeMapObject(mapleMapItem);
        }
      }
  ...
  public static void ChangePetBuff(final LittleEndianAccessor slea, final MapleCharacter chr)
  {
    ...
    if (type == 0 && mode == 0 && skillsize == 0)
    {
      ...
      chr.setKeyValue(9999, "skillid", String.valueOf(skillId));
    }
    else if (type == 0 && skillsize == 1)
    {
      pet.setBuffSkillId2(skillId);
      chr.setKeyValue(9999, "skillid2", String.valueOf(skillId));
    }
    else if (type == 1 && skillsize == 0)
    {
      pet.setBuffSkillId(skillId);
      chr.setKeyValue(9999, "skillid3", String.valueOf(skillId));
    }
    else if (type == 1 && skillsize == 1)
    {
      pet.setBuffSkillId2(skillId);
      chr.setKeyValue(9999, "skillid4", String.valueOf(skillId));
    }
    else if (type == 2 && skillsize == 0)
    {
      pet.setBuffSkillId(skillId);
      chr.setKeyValue(9999, "skillid5", String.valueOf(skillId));
    }
    else if (type == 2 && skillsize == 1)
    {
      pet.setBuffSkillId2(skillId);
      chr.setKeyValue(9999, "skillid6", String.valueOf(skillId));
    }
