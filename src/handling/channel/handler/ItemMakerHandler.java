...
public class ItemMakerHandler
{
    ...
    switch (makerType)
    {
      case 1:
        ...
        if (GameConstants.isGem(toCreate))
        {
          ...
          MapleInventoryManipulator.addById(c, randGemGiven, (byte) ((taken == randGemGiven) ? 9 : 1), "Made by Gem " + toCreate + " on " + FileoutputUtil.CurrentReadable_Date());
      ...
      case 4:
        ...
        if (!ii.isDropRestricted(itemId) && !ii.isAccountShared(itemId))
        {
          ...
          MapleInventoryManipulator.addById(c, arrayOfInt[0], (byte) arrayOfInt[1], "Made by disassemble " + itemId + " on " + FileoutputUtil.CurrentReadable_Date());
  ...
  private static final int getRandomGem(List<Pair<Integer, Integer>> rewards)
  {
    ...
    {
      int itemid = p.getLeft().intValue();
      for (int i = 0; i < p.getRight().intValue(); i++)
      {
        items.add(Integer.valueOf(itemid));
      }
    }
    return items.get(Randomizer.nextInt(items.size())).intValue();
  }
  ...
  private static final int checkRequiredNRemove(MapleClient c, List<Pair<Integer, Integer>> recipe)
  {
    ...
    {
      if (!c.getPlayer().haveItem(p.getLeft().intValue(), p.getRight().intValue(), false, true))
      {
        return 0;
      }
    }
    for (Pair<Integer, Integer> p : recipe)
    {
      itemid = p.getLeft().intValue();
      MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType(itemid), itemid, p.getRight().intValue(), false, false);
    }
    return itemid;
  }
  ...
  public static final void CraftComplete(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr)
  {
    ...
    else
    {
      ...
      {
        if (!chr.haveItem(e.getKey().intValue(), e.getValue().intValue()))
        {
          return;
        }
      }
      ...
      {
        if (!MapleInventoryManipulator.checkSpace(c, triple.left.intValue(), triple.mid.intValue(), ""))
        {
          return;
        }
      }
      for (Map.Entry<Integer, Integer> e : ce.reqItems.entrySet())
      {
        MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType(e.getKey().intValue()), e.getKey().intValue(), e.getValue().intValue(), false, false);
      }
      if (Randomizer.nextInt(100) < 100 - (ce.reqSkillLevel - theLevl) * 20 || craftID / 10000 <= 9201)
      {
        ...
        while (true)
        {
          ...
          for (Triple<Integer, Integer, Integer> triple : ce.targetItems)
          {
            if (Randomizer.nextInt(100) < triple.right.intValue())
            {
              toGet = triple.left.intValue();
              quantity = triple.mid.shortValue();
              Item receive = null;
              cr = CraftRanking.COOL;
              ...
              if (ce.period > 0)
              {
                receive.setExpiration(System.currentTimeMillis() + (ce.period * 60000L));
              }
