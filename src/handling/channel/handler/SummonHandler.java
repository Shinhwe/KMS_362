  ...
  public static final void SubSummon(final LittleEndianAccessor slea, final MapleCharacter chr)
  {
    ...
    switch (sum.getSkill())
    {
      case 5210015:
      {
        ...
        atoms.add(new SecondAtom(0x22, chr.getId(), mobid, 5201017, 4000, 0, 1, new Point(sum.getTruePosition().x, sum.getTruePosition().y), Collections.singletonList(0)));
        atoms.add(new SecondAtom(0x22, chr.getId(), mobid, 5201017, 4000, 0, 1, new Point(sum.getTruePosition().x, sum.getTruePosition().y), Collections.singletonList(0)));
      ...
      case 25121133:
      {
        ...
        chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 25121209, 2, 0, 25121133, (byte) (chr.isFacingLeft() ? 1 : 0), true, null, null, null));
      ...
      case 400041038:
      {
        ...
        final List<MapleMapObject> objs = chr.getMap().getMapObjectsInRange(sum.getTruePosition(), 500000.0, Collections.singletonList(MapleMapObjectType.MONSTER));
        ...
        if (monsters.size() > 0)
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, sum.getSkill(), sum.getSkill(), 4, 0, 0, (byte) 0, true, null, null, null));
