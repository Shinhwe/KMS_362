...
public class MapleGuildRanking
{
  ...
  private static final long lastReloadTime = 0L;
  private final List<GuildRankingInfo> honorRank = new LinkedList<>();
  ...
  private void reload()
  {
    ...
    {
      this.honorRank.add(new GuildRankingInfo(list.getLeft(), list.getMid().intValue(), list.getRight().intValue()));
    }
    for (Triple<String, Integer, Integer> list : culvertRanks)
    {
      this.culvertRank.add(new GuildRankingInfo(list.getLeft(), list.getMid().intValue(), list.getRight().intValue()));
    }
  }
