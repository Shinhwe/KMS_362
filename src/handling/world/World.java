    ...
    public static void updateParty(final int partyid, final PartyOperation operation, final MaplePartyCharacter target)
    {
      ...
      if (operation == PartyOperation.LEAVE || operation == PartyOperation.EXPEL)
      {
        ...
        if (chz > 0)
        {
          ...
          {
            chr.setParty(null);
            chr.getClient().getSession().writeAndFlush(CWvsContext.PartyPacket.updateParty(chr.getClient().getChannel(), party, operation, target));
          }
        }
      ...
      for (final MaplePartyCharacter partychar : party.getMembers())
      {
        ...
        chr2.getClient().getSession().writeAndFlush(CWvsContext.PartyPacket.updateParty(chr2.getClient().getChannel(), party, operation, target));
    ...
    public static void messengerChatNow(final int messengerid, final String charname, final String text, final String namefrom)
    {
      ...
      for (final MapleMessengerCharacter messengerchar : messenger.getMembers())
      {
        if (messengerchar != null && !messengerchar.getName().equals(namefrom))
        {
          ...
          chr.getClient().getSession().writeAndFlush(CField.messengerChatNow(chr, messengerid, charname, text));
        }
      }
    }
    ...
    public static List<MapleGuild> getGuildsByName(String name, boolean option, byte type)
    {
      ...
      for (MapleGuild g : guilds.values())
      {
        ...
        if (option)
        {
          ...
          {
            /*  794 */
            if (g.getName().contains(name)) /*  795 */
            {
              ret.add(g);
            }
            continue;
          ...
          {
            /*  798 */
            if (g.getLeaderName().contains(name)) /*  799 */
            {
              ret.add(g);
            }
            continue;
          ...
          if (type == 3)
          {
            ...
            /*  805 */
            if (g.getLeaderName().matches(name)) /*  806 */
            {
              ret.add(g);
            }
          }
        ...
        {
          /*  811 */
          if (g.getName().contains(name)) /*  812 */
          {
            ret.add(g);
          }
          continue;
        ...
        {
          /*  815 */
          if (g.getLeaderName().contains(name)) /*  816 */
          {
            ret.add(g);
          }
          continue;
