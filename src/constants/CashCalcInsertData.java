  ...
  public static void main(String[] args)
  {
    ...
    for (int i = 0; i < strs.length; i++)
    {
      ...
      for (Pair<Integer, String> itemPair : MapleItemInformationProvider.getInstance().getAllItems())
      {
          ...
          {
              retItems.add(new Triple<>(String.valueOf(itemPair.getLeft()), itemPair.getRight(), data[1]));
          }
      }
      ...
      List<Triple<String, String, String>> itemcodes = new ArrayList<>();
        if (retItems != null && retItems.size() > 0)
        {
            if (retItems.size() != 1)
            {
                ...
                {
                    System.out.println((z + 1) + ". " + ((Triple) retItems.get(z)).getLeft() + " - " + ((Triple) retItems.get(z)).getMid());
                }
                int geti = sc.nextInt();
                if (geti != -1)
                {
