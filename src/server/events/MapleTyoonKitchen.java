...
public class MapleTyoonKitchen
{
  ...
  private static final List<MapleCharacter> Matchinglist = new ArrayList<>();
  private static final List<MapleCharacter> Playerlist = new ArrayList<>();
        ...
        for (MaplePartyCharacter pm : chr.getParty().getMembers())
        {
          if (pm.getPlayer() != null)
          {
            ...
            for (Pair<SecondaryStat, SecondaryStatValueHolder> data : pm.getPlayer().getEffects())
            {
              ...
              {
                pm.getPlayer().cancelEffect(mbsvh.effect, Collections.singletonList(data.left));
              }
            }
