...
public class MapleBattleGround
{
      ...
      case 80003002:
        ...
        for (Pair<Integer, Integer> cooll : coollist)
        {
          if (c.getPlayer().skillisCooling(cooll.getLeft().intValue()))
          {
            ...
            {
              c.send(BattleGroundPacket.CoolDown(80003005, cooll.getRight().intValue() * 1000, (int) ((cooll.getRight().intValue() * 1000) - c.getPlayer().getCooldownLimit(cooll.getLeft().intValue()))));
            }
          }
        }
        break;
  ...
  public static void AttackRefresh(LittleEndianAccessor slea, MapleClient c)
  {
    ...
    if (stype == 1)
    {
      ...
      c.getPlayer().getMap().broadcastMessage(BattleGroundPacket.AttackRefresh(c.getPlayer(), stype, count, type, 0));
    }
    else if (stype == 2)
    {
  ...
  public static void AttackDamage(LittleEndianAccessor slea, MapleClient c)
  {
    ...
    for (i = 0; i < peoplecount; i++)
    {
      ...
      if (chred != null)
      {
        ...
        else if (chred.getChr().getBuffedValue(80002672))
        {
          ...
          if (c.getPlayer().getBattleGroundChr().getHp() <= 0 && c.getPlayer().getBattleGroundChr().isAlive())
          {
            ...
            else
            {
              ...
              int rank = alivechr + 2;
              c.getPlayer().addKV("BattlePVPRank", String.valueOf(rank));
              c.getPlayer().addKV("BattlePVPLevel", String.valueOf(c.getPlayer().getBattleGroundChr().getLevel()));
              c.getPlayer().addKV("BattlePVPKill", String.valueOf(c.getPlayer().getBattleGroundChr().getKill()));
              if (alivechr == 0)
              {
                chred.getChr().addKV("BattlePVPRank", "1");
                chred.getChr().addKV("BattlePVPLevel", String.valueOf(chred.getLevel()));
                chred.getChr().addKV("BattlePVPKill", String.valueOf(chred.getKill()));
        ...
        if (!BattleGroundGameHandler.isNotDamage())
        {
          ...
          minfo.add(minfo2);
          c.getPlayer().getMap().broadcastMessage(BattleGroundPacket.AttackDamage(c.getPlayer(), pos1, pos2, pos3, pos4, pos5, minfo, true, attackcount, skillid, unk1, unk2, unk, unk4, unk5, unk6, unk7, unk8,
          ...
          if (chred.getHp() <= 0 && chred.isAlive())
          {
            ...
            else
            {
              ...
              int rank = alivechr + 2;
              chred.getChr().addKV("BattlePVPRank", String.valueOf(rank));
              chred.getChr().addKV("BattlePVPLevel", String.valueOf(chred.getLevel()));
              chred.getChr().addKV("BattlePVPKill", String.valueOf(chred.getKill()));
              if (alivechr == 0)
              {
    ...
    for (i = 0; i < mobcount; i++)
    {
      ...
      if (monster != null)
      {
        ...
        minfo.add(minfo2);
        c.getPlayer().getMap().broadcastMessage(BattleGroundPacket.AttackDamage(c.getPlayer(), pos1, pos2, pos3, pos4, pos5, minfo, false, attackcount, skillid, unk1, unk2, unk, unk4, unk5, unk6, unk7, unk8,
