    ...
    implements Serializable
{
  ...
  private final boolean energyChargeActived = false;
  ...
  public static int parseEval(String data, int level)
  {
    ...
    if (dddd.charAt(0) == '-')
    {
      dddd = dddd.charAt(1) == 'u' || dddd.charAt(1) == 'd' ? "n(" + dddd.substring(1) + ")" : "n" + dddd.substring(1);
    }
    else if (dddd.charAt(0) == '=')
    {
  ...
  private static final int parseEval(String path, MapleData source, int def, String variables, int level)
  {
    ...
    if (dddd.length() >= 3 && dddd.startsWith("log"))
    {
      ...
      dddd = base + dddd.substring(level >= 10 ? 7 : 6);
    }
    else if (dddd.charAt(0) == '-')
    {
      dddd = dddd.charAt(1) == 'u' || dddd.charAt(1) == 'd' ? "n(" + dddd.substring(1) + ")" : "n" + dddd.substring(1);
  ...
  public final boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos, int localDuration, byte rltype, boolean showEffect)
  {
    block196:
    {
      block197:
      {
        block195:
        {
          ...
          long mpchange = this.calcMPChange(applyfrom, this.sourceid == 36101001 || primary);
          ...
          if (mpchange != 0L && applyto.isAlive() && !noCon && applyto.getBattleGroundChr() == null)
          {
            ...
            if (mpchange < 0L || !GameConstants.isDemonSlayer(applyto.getJob()))
            {
          ...
          if ((summonMovementType = this.getSummonMovementType()) != null && ((this.sourceid == 400021071 || this.sourceid == 35111008 || this.sourceid == 35120002 || this.sourceid == 400021092) && primary || this.sourceid != 400021071 && this.sourceid != 35111008 && this.sourceid != 35120002 && this.sourceid != 400021092))
          {
            ...
            else if (this.sourceid != 35111002 && this.sourceid != 400021047 && this.sourceid != 400011065 && this.sourceid != 14000027 && this.sourceid != 14100027 && this.sourceid != 0xD74D4D && this.sourceid != 14120008 && this.sourceid != 151100002 && primary)
            {
              ...
              {
                boolean cancels = applyto.getBuffedEffect(SecondaryStat.SoulMP) == null || applyto.getBuffedEffect(SecondaryStat.SoulMP).getSourceId() != this.sourceid;
                if (cancels)
                {
            ...
            if (spawn)
            {
              ...
              else if (this.sourceid == 400051017)
              {
                ...
                List<MapleMapObject> objs = applyfrom.getMap().getMapObjectsInRange(applyto.getTruePosition(), 500000.0, Collections.singletonList(MapleMapObjectType.MONSTER));
        ...
        boolean spawn = (this.sourceid == 400031039 || this.sourceid == 400031040) && primary || (this.sourceid == 400040008 || this.sourceid == 400041008) && !primary || this.sourceid == 2111003 || (this.sourceid == 12121005 || this.sourceid == 400001017 || this.sourceid == 100001261 || this.sourceid == 151121041 || this.sourceid == 4221006) && primary || this.sourceid != 400031039 && this.sourceid != 400031040 && this.sourceid != 12121005 && this.sourceid != 400001017 && this.sourceid != 100001261 && this.sourceid != 151121041 && this.sourceid != 400040008 && this.sourceid != 400041008 && this.sourceid != 4221006;
        if (!spawn)
        {
          break block196;
        }
    ...
    if (this.rewardItem != null && this.totalprob > 0)
    {
      for (Triple<Integer, Integer, Integer> reward : this.rewardItem)
      {
        ...
        MapleInventoryManipulator.addById(applyto.getClient(), reward.left, reward.mid.shortValue(), "Reward item (effect): " + this.sourceid + " on " + FileoutputUtil.CurrentReadable_Date());
      }
    }
    return true;
  }
  ...
  public final void silentApplyBuff(MapleCharacter chr, long starttime, Map<SecondaryStat, Pair<Integer, Integer>> statup, int cid)
  {
    ...
    for (Map.Entry<SecondaryStat, Pair<Integer, Integer>> statupz : statup.entrySet())
    {
      ...
      SummonMovementType summonMovementType;
      long remainDuration = (long) statupz.getValue().right.intValue() - (System.currentTimeMillis() - starttime);
  ...
  private final void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, int newDuration, Point pos, boolean showEffect)
  {
    ...
    switch (this.sourceid)
    {
      ...
      case 400001052:
      {
        localstatups.put(SecondaryStat.NewFlying, new Pair<Integer, Integer>(1, (int) SkillFactory.getSkill(400001007).getEffect(this.level).t * 1000));
        break;
      }
      case 80003046:
      {
...
//                System.out.println("버프 종착점 : " + SkillFactory.getSkill(this.sourceid).getName() + " / " + SkillFactory.getSkill(this.sourceid).getId());
  ...
  public void giveAnotherBuff(MapleCharacter applyto, MapleCharacter applyfrom, Map<SecondaryStat, Pair<Integer, Integer>> localstatups, boolean BuffTimeR)
  {
    ...
    for (Map.Entry<SecondaryStat, Pair<Integer, Integer>> statup : localstatups.entrySet())
    {
      ...
      addV.add(new Pair<SecondaryStat, SecondaryStatValueHolder>(statup.getKey(), new SecondaryStatValueHolder(this, starttime, statup.getValue().left, statup.getValue().right, applyfrom.getId(), indieList1, indieList2)));
    }
    if (exit)
    {
      applyto.getClient().getSession().writeAndFlush(CWvsContext.enableActions(applyto, true, false));
...
//            localCooltime -= localCooltime * SkillFactory.getSkill(71000231).getEffect(chra.getSkillLevel(71000231)).coolTimeR / 100;
  ...
  { // 쿨타임
    ...
    else if (cooltimeMS > 0)
    {
      ...
      if (cooltimeMS > 10000)
      {
        localCooltime = cooltimeMS - (chra.getStat().reduceCooltime * 1000);
      }
      else
      {
