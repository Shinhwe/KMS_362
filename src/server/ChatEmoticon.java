...
{
  private static final Map<Integer, List<Integer>> emoticons = new ConcurrentHashMap<>();
  ...
  public static void LoadChatEmoticons(MapleCharacter chr, List<MapleChatEmoticon> ems)
  {
    for (MapleChatEmoticon em : ems)
    {
      ...
      {
        int e = iterator.next().intValue();
        short slot = 0;
        for (Pair<Integer, Short> a : em.getBookmarks())
        {
          if (a.left.intValue() == e && a.right.shortValue() > 0)
          {
            slot = a.right.shortValue();
          }
        }
