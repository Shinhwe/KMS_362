...
public class BingoGame
{
  ...
  private final Map<MapleCharacter, int[][]> players = new HashMap<>();
  private final List<MapleCharacter> rank = new ArrayList<>();
  private final List<Integer> hostednumbers = new ArrayList<>();
  private final int round = 1;
  private final int MessageTime = 3;
    ...
    for (MapleCharacter chr : this.players.keySet())
    {
      ...
      if (chr != null)
      {
        ...
        if (!this.rank.contains(chr))
        {
          ...
          {
            if (chr.getInventory(GameConstants.getInventoryType(item.left.intValue())).getNumFreeSlot() > 0)
            {
              chr.gainItem(item.left.intValue(), item.right.intValue());
              continue;
            }
            chr.dropMessage(-7, "인벤토리 공간이 부족하여 " + MapleItemInformationProvider.getInstance().getName(item.left.intValue()) + "아이템 " + item.right + "개를 지급받지 못했습니다. 이 메세지를 캡쳐하여 GM에게 문의해주세요.");
          }
        }
      }
    }
