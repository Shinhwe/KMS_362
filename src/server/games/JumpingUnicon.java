  ...
  public static void StartGame()
  {
    ...
    if (!MapleMap.uniconflower.isEmpty())
    {
      for (Pair<Integer, Point> a : MapleMap.uniconflower)
      {
        ...
        {
          Item idrop = new Item(a.left.intValue(), (short) 0, (short) 1, 0);
          MapleMapItem mdrop = new MapleMapItem(idrop, a.right, chr, chr, (byte) 0, false, 0);
          ...
          for (MapleCharacter chr2 : map.getAllChracater())
          {
            if (chr.getSkillCustomValue0(993074001) == chr2.getSkillCustomValue0(993074001))
            {
              ...
              chr2.getClient().send(CField.dropItemFromMapObject(map, mdrop, a.right, a.right, (byte) 2, false));
            }
          }
        }
      }
    ...
    Timer.EtcTimer.getInstance().schedule(() ->
    {
      ...
      if (MatchingQueue.size() >= n)
      {
        for (MapleCharacter chr : MatchingQueue)
        {
          ...
          {
            if (SkillFactory.getSkill(buff.getRight().effect.getSourceId()) != null)
            {
              allBuffs.add(buff);
            }
          }
