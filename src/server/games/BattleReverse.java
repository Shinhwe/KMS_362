...
public class BattleReverse
{
  ...
  private final List<BattleReversePlayer> Players;
  private final BattleReverseStone[][] Board;
  ...
  public BattleReverse(final List<MapleCharacter> chrs)
  {
    ...
    {
      this.Players.add(new BattleReversePlayer(chr, 10000, stone++));
    }
  }
  ...
  private void InitBoard()
  {
    for (int a = 0; a < 8; ++a)
    {
      for (int b = 0; b < 8; ++b)
      {
        this.Board[a][b] = new BattleReverseStone(a, b, -1);
      }
    }
  ...
  public void setTurn(final MapleCharacter mapleCharacter, final BattleReversePlayer battleReversePlayer, final BattleReversePlayer battleReversePlayer2, final Point point)
  {
    if (this.getPlaceablePoints(this.getOpponent(mapleCharacter.getId())).size() != 0)
    {
      ...
      {
        battleReversePlayer3.chr.getClient().getSession().writeAndFlush(SLFCGPacket.MultiOthelloGamePacket.onBoardUpdate(battleReversePlayer2.getPlayer().getId() == battleReversePlayer3.getPlayer().getId(), point, battleReversePlayer.getStoneId(), this.getOpponent(battleReversePlayer.getPlayer().getId()).getHP(), this.getStones(), (byte) 1));
      }
    ...
    if (this.getPlaceablePoints(this.getPlayer(mapleCharacter.getId())).size() == 0)
    {
      for (final BattleReversePlayer battleReversePlayer4 : this.Players)
      {
        battleReversePlayer4.chr.getClient().getSession().writeAndFlush(SLFCGPacket.MultiOthelloGamePacket.onBoardUpdate(battleReversePlayer2.getPlayer().getId() == battleReversePlayer4.getPlayer().getId(), point, battleReversePlayer.getStoneId(), this.getOpponent(battleReversePlayer.getPlayer().getId()).getHP(), this.getStones(), (byte) 1));
      }
      this.endGame(mapleCharacter, false);
      return;
    }
