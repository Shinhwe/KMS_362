...
public class DetectiveGame
{
  ...
  private final List<MapleCharacter> Rank = new ArrayList<>();
  private final List<MapleCharacter> Fail = new ArrayList<>();
  private final Map<MapleCharacter, Integer> Players = new HashMap<>();
  private final Map<MapleCharacter, Integer> Answers = new HashMap<>();
  private final ScheduledFuture<?> DetectiveTimer = null;
  private int Stage = 1;
  ...
  {
    this.Players.put(chr, Integer.valueOf(this.Players.get(chr).intValue() + 1));
    int temp = this.Players.get(chr).intValue();
    if (this.Players.get(chr).intValue() == 15)
    {
      this.Fail.add(chr);
    }
  }
  ...
  {
    return this.Answers.get(chr).intValue();
  }
  ...
  private int GetRandomNumber()
  {
    ...
    int num = temp.get(0).intValue() * 100 + temp.get(1).intValue() * 10 + temp.get(2).intValue();
