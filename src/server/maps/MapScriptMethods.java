  ...
  public static void startScript_User(final MapleClient c, final String scriptName)
  {
    ...
    switch (onUserEnter.fromString(scriptName))
    {
      ...
      case mPark_stageEff:
      {
        ...
        switch (c.getPlayer().getMapId() % 1000 / 100)
        {
          ...
          case 3:
          {
            ...
            c.getSession().writeAndFlush(CField.showEffect("monsterPark/stageEff/number/" + (c.getPlayer().getMapId() % 1000 / 100 + 1)));
            break;
          }
          case 4:
          {
      ...
      case explorationPoint:
      {
        ...
        if (m != null && c.getPlayer().getLevel() >= m.level && c.getPlayer().getQuestStatus(m.questid) != 2)
        {
          ...
          for (int l = 0; l < m.maps.length; ++l)
          {
            ...
            if (c.getPlayer().getMapId() == m.maps[l] && quest.charAt(l + 6) == '0')
            {
              sb3.append("1");
              changed = true;
              changedd = true;
      ...
      case enter_993001000:
      {
        ...
        c.getPlayer().setKeyValue(18838, "count", String.valueOf(c.getPlayer().getKeyValue(18838, "count")));
        c.getPlayer().setKeyValue(18838, "stage", String.valueOf(records.size()));
        for (int a = 0; a < records.size(); ++a)
        {
          final PlatformerRecord rec = records.get(a);
          c.getPlayer().setKeyValue(18839 + a, "isClear", "1");
          c.getPlayer().setKeyValue(18839 + a, "br", String.valueOf(rec.getClearTime()));
          c.getPlayer().setKeyValue(18839 + a, "cs", String.valueOf(rec.getStars()));
        }
        break;
      }
