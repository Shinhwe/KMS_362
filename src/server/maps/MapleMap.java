...
public final class MapleMap
{
  ...
  private final String speedRunLeader = "";
  private final List<Integer> dced = new ArrayList<>();
  private final long speedRunStart = 0L;
  private final Map<Integer, List<Integer>> monsterDefense = new LinkedHashMap<>();
  ...
  public int eliteChampionCount = 0;
  public int eliteChampionType = 0;
  ...
  private boolean isEliteBossMap;
  private boolean isEliteBossRewardMap;
  private boolean isEliteChampionMap;
  private boolean eliteChampionFinal;
  ...
  public void MapiaMorning (final MapleCharacter player)
  {
    ...
    m_timer.schedule(m_task, (this.aftertime * 1000L));
  }
  ...
  public void MapiaVote (final MapleCharacter player)
  {
    ...
    else
    {
      ...
      m_timer.schedule(m_task, (this.votetime * 1000L));
    }
  }
  ...
  public void MapiaNight (final MapleCharacter player)
  {
    ...
    m_timer.schedule(m_task, 3000L, (this.nighttime * 1000L));
  }
  ...
  public boolean isLevelMob (MapleCharacter chr)
  {
  ...
  private void dropFromMonster (MapleCharacter chr, MapleMonster mob, boolean instanced)
  {
    ...
    if (mob.isEliteBoss())
    {
      customDropList.add(new MonsterDropEntry(2432970, 1000000, 5, 10, 0, 1));
      customDropList.add(new MonsterDropEntry(2432398, 1000000, 1, 2, 0, 1));
      customDropList.add(new MonsterDropEntry(5062009, 1000000, 20, 40, 0, 1));
      customDropList.add(new MonsterDropEntry(5062010, 1000000, 10, 20, 0, 1));
      customDropList.add(new MonsterDropEntry(5062500, 1000000, 5, 10, 0, 1));
      customDropList.add(new MonsterDropEntry(2048717, 1000000, 5, 10, 0, 1));
      customDropList.add(new MonsterDropEntry(4001832, 1000000, 3000, 5000, 0, 1));
    }
    if (mob.isEliteMonster())
    {
      customDropList.add(new MonsterDropEntry(2432970, 1000000, 1, 2, 0, 1));
      customDropList.add(new MonsterDropEntry(5062009, 1000000, 5, 10, 0, 1));
      customDropList.add(new MonsterDropEntry(5062010, 1000000, 2, 5, 0, 1));
      customDropList.add(new MonsterDropEntry(5062500, 1000000, 1, 1, 0, 1));
      customDropList.add(new MonsterDropEntry(2048716, 1000000, 1, 2, 0, 1));
    ...
    if (privateDropList.size() > 0)
    {
      ...
      if (chr.getParty() != null)
      {
        ...
        for (MapleCharacter member : memberList)
        {
          ...
          if (mob.getStats().isBoss())
          {
            System.out.println("BOSS[" + mob.getId() + "]的獨立掉落计算结束, 是否隊伍 = 是, 角色Id = " + member.getId() + " , 掉落列表 = " + JSON.toJSONString(finalPrivateDropList));
          }

          if (mob.isEliteMonster())
          {
            System.out.println("精英怪[" + mob.getId() + "]的獨立掉落计算结束, 是否隊伍 = 是, 角色Id = " + member.getId() + " , 掉落列表 = " + JSON.toJSONString(finalPrivateDropList));
          }

          if (mob.isEliteBoss())
          {
            System.out.println("精英BOSS[" + mob.getId() + "]的獨立掉落计算结束, 是否隊伍 = 是, 角色Id = " + member.getId() + " , 掉落列表 = " + JSON.toJSONString(finalPrivateDropList));
          }
      ...
      else
      {
        ...
        if (mob.getStats().isBoss())
        {
          System.out.println("BOSS[" + mob.getId() + "]的獨立掉落计算结束, 是否隊伍 = 不是, 掉落列表 = " + JSON.toJSONString(finalPrivateDropList));
        }

        if (mob.isEliteMonster())
        {
          System.out.println("精英怪[" + mob.getId() + "]的獨立掉落计算结束, 是否隊伍 = 不是, 掉落列表 = " + JSON.toJSONString(finalPrivateDropList));
        }

        if (mob.isEliteBoss())
        {
          System.out.println("精英BOSS[" + mob.getId() + "]的獨立掉落计算结束, 是否隊伍 = 不是, 掉落列表 = " + JSON.toJSONString(finalPrivateDropList));
        }
        ...
        if (finalPrivateDropList.size() > 0)
        {
          ...
          for (MonsterDropEntry de : finalPrivateDropList)
          {
            currentDropIndex += 1;
            // 不是独立掉落按照击杀者计算概率
            drop(de, mob, chr, pos, currentDropIndex, finalPrivateDropList.size());
          }
        }
      }
    }
  ...
  public void drop (MonsterDropEntry de, MapleMonster mob, MapleCharacter chr, Point dropPosition, int currentDropIndex, int totalDropCount)
  {
    MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();

    byte droptype = 0;
    if (mob.getStats().isExplosiveReward() && mob.getStats().isBoss())
    {
      droptype = 3;
    }
    ...
    if (totalDropCount > 5)
    {
      int a = totalDropCount / 2;
      if (currentDropIndex < a)
      {
        poss.x -= 30 * (a - currentDropIndex);
      }
      else if (currentDropIndex >= a)
      {
        poss.x += 30 * (currentDropIndex - a);
      }
      dropPosition = poss;
    }
    else if (droptype == 3)
    {
      dropPosition.x = mobpos + ((currentDropIndex % 2 == 0) ? (40 * (currentDropIndex + 1) / 2) : -(40 * currentDropIndex / 2));
    }
    else
    {
      dropPosition.x = mobpos + ((currentDropIndex % 2 == 0) ? (20 * (currentDropIndex + 1) / 2) : -(20 * currentDropIndex / 2));
    }

    MapleFoothold fh2 = this.footholds.findBelow(dropPosition);

    if (fh2 != null)
    {
      if (fh2.getX1() > dropPosition.x)
      {
        dropPosition.x = fh2.getX1();
      }
      else if (fh2.getX2() < dropPosition.x)
      {
        dropPosition.x = fh2.getX2();
      }
    }
    if (de.itemId == 0)
    {
      ...
      if (mobLevel == 1)
      {
        ...
        maximumMesosDropped = 1;
      }
      else if (mobLevel > 1 && mobLevel <= 20)
      {
        minimumMesosDropped = (int) Math.round(mobLevel * 1.6);
        maximumMesosDropped = (int) Math.round(mobLevel * 2.4);
      }
      else if (mobLevel > 20 && mobLevel <= 30)
      {
        minimumMesosDropped = Math.round(mobLevel * 2);
        maximumMesosDropped = Math.round(mobLevel * 3);
      }
      else if (mobLevel > 30 && mobLevel <= 40)
      {
        minimumMesosDropped = (int) Math.round(mobLevel * 2.4);
        maximumMesosDropped = (int) Math.round(mobLevel * 3.6);
      }
      else if (mobLevel > 40 && mobLevel <= 50)
      {
        minimumMesosDropped = (int) Math.round(mobLevel * 2.8);
        maximumMesosDropped = (int) Math.round(mobLevel * 4.2);
      }
      else if (mobLevel > 50 && mobLevel <= 60)
      {
        minimumMesosDropped = Math.round(mobLevel * 4);
        maximumMesosDropped = Math.round(mobLevel * 6);
      }
      else if (mobLevel > 60 && mobLevel <= 70)
      {
        minimumMesosDropped = (int) Math.round(mobLevel * 4.8);
        maximumMesosDropped = (int) Math.round(mobLevel * 7.2);
      }
      else if (mobLevel > 70 && mobLevel <= 80)
      {
      ...
      if (mesos > 0)
      {
        ...
        {
          spawnMobMesoDrop(mesos, calcDropPos(dropPosition, mob.getTruePosition()), mob, chr, false, droptype);
        }
      }
    }
  ...
  public void killMonster (final MapleMonster monster, final MapleCharacter chr, boolean withDrops, boolean second, byte animation, int lastSkill)
  {
    ...
    else if (partyquest == 2)
    {
      ...
      if (moonstak % 2 == 0)
      {
        ...
        // 위 두줄 같음
        MapleMonster mob;
        for (MapleMapObject monstermo : getMapObjectsInRange(chr.getPosition(), Double.POSITIVE_INFINITY, Collections.singletonList(MapleMapObjectType.MONSTER)))
        {
    ...
    try
    {
      ...
      for (String qlis : qlist)
      {
        if (chr.getV(qlis + "_" + monster.getId() + "_isclear") != null && chr.getV(qlis + "_" + monster.getId() + "_count") != null && chr.getV(qlis + "_" + monster.getId() + "_mobq") != null)
        {
          if (chr.getV(qlis + "_" + monster.getId() + "_isclear").equals("0") && Integer.parseInt(chr.getV(qlis + "_" + monster.getId() + "_count")) < Integer.parseInt(chr.getV(qlis + "_" + monster.getId() + "_mobq")))
          {
            ...
            chr.addKV(qlis + "_" + monster.getId() + "_count", String.valueOf(count));
      ...
      String questionCustomData = chr.getQuest(MapleQuest.getInstance((int) chr.getKeyValue(51351, "startquestid"))).getCustomData();
      if (isEliteChampionMap())
      {
        boolean end = false;
        boolean next = isEliteChampionFinal();
        List<Integer> killmoblist = new ArrayList<>();
        if (getEliteChampionType() == 0)
        {
          for (int i = 0; i <= 5; i++)
          {
        ...
        else if (getEliteChampionType() == 1)
        {
          for (int i = 0; i <= 2; i++)
          {
        ...
        else if (getEliteChampionType() == 2)
        {
          for (int i = 0; i <= 1; i++)
          {
        ...
        else if (getEliteChampionType() == 3)
        {
          for (int i = 0; i <= 1; i++)
          {
        ...
        else if (getEliteChampionType() == 4)
        {
          for (int i = 0; i <= 1; i++)
          {
        ...
        for (Integer list : killmoblist)
        {
          ...
          {
            setEliteChampionCount(getEliteChampionCount() - 1);
            if (getEliteChampionCount() <= 0)
            {
              end = true;
              break;
            }
        ...
        if (!next && end)
        {
          if (getEliteChampionType() == 2)
          {
            setEliteChampionCount(1);
            setEliteChampionFinal(true);
            int i = 8220123;
        ...
        {
          setEliteChampionMap(false);
          setEliteChampionCount(0);
          setEliteChampionType(0);
          this.eliteCount++;
          ...
          if (!monster.isEliteMonster())
          {
            ...
            if ((chr.getKeyValue(51351, "startquestid") == 49012L || chr.getKeyValue(51351, "startquestid") == 49013L || chr.getKeyValue(51351, "startquestid") == 49014L) && questionCustomData != null)
            {
              int maxmob = (chr.getKeyValue(51351, "startquestid") == 49012L) ? 1 : ((chr.getKeyValue(51351, "startquestid") == 49013L) ? 2 : 3);
              String q = "";
              int count = Integer.parseInt(questionCustomData);
              count++;
              if (count < 10)
              {
                q = "00" + count;
      ...
      if (chr.getClient().getQuestStatus(100825) == 2)
      {
        ...
        {
          if (!monster.getStats().isBoss() && !monster.isEliteMonster() && !monster.getIsEliteChampion() && !monster.isEliteBoss())
          {
          ...
          if (chr.getBuffedValue(80003064) && chr.getSkillCustomValue0(100857) <= 0L)
          {
            ...
            else if (10L - chr.getKeyValue(100857, "feverCnt") > 0L || chr.isGM())
            {
              ...
              chr.setKeyValue(100857, "count", String.valueOf(chr.getKeyValue(100857, "count") + (1L)));
              ...
              else if (chr.getKeyValue(100857, "count") >= 500L)
              {
                ...
                {
                  chr.getClient().setCustomKeyValue(501567, "fever", String.valueOf(chr.getClient().getCustomKeyValue(501567, "fever") + 1L));
                }
                chr.getClient().send(CField.enforceMSG("메이플 팡팡 리액션 타임! 어서 후원받은 팡팡 주머니를 터뜨려!", 341, 20000));
                chr.getClient().send(CField.enforceMsgNPC(9062549, 3000, "후원이 터지는 이 순간! 멋진 #r팡팡 리액션#k을 보여줘!"));
                chr.setKeyValue(100857, "count", "0");
                chr.setKeyValue(100857, "feverCnt", String.valueOf(chr.getKeyValue(100857, "feverCnt") + 1L));
                ...
                Timer.EtcTimer.getInstance().schedule(() ->
                {
                  ...
                  Timer.EtcTimer.getInstance().schedule(() ->
                  {
                    if (chr.getSkillCustomValue(100857) != null)
                    {
                      ...
                      chr.getClient().send(CField.UIPacket.detailShowInfo("오늘의 리액션 횟수 : " + chr.getKeyValue(100857, "feverCnt") + "/10 회", 3, 20, 20));
                      chr.dropMessage(5, "오늘의 리액션 횟수 : " + chr.getKeyValue(100857, "feverCnt") + "/10 회");
      ...
      if (getIsEliteBossRewardMap() && getEliteBossRewardType() == 1 && monster.getId() == 8220027 && Randomizer.isSuccess(chr.isGM() ? 100 : 60))
      {
      ...
      else if (!getIsEliteBossMap() && !getIsEliteBossRewardMap() && monster.isEliteMonster())
      {
      ...
      if (getCustomValue(8222222) == null && EliteMobCommonCount > 200 && Randomizer.isSuccess(EliteMobCommonCount, 50000))
      {
        this.EliteMobCommonCount = 0;
        if ((getEliteCount() == 5 || getEliteCount() == 10 || getEliteCount() == 15) && monster.getEliteGrade() <= -1 && isSpawnPoint() && !isEliteChampionMap() && !getIsEliteBossMap() && !getIsEliteBossRewardMap())
        {
          ...
          int j = 0;
          String txt = "";
          setEliteChampionMap(true);
          setEliteChampionType(type);
          setCustomInfo(8222222, 0, 5 * 60 * 1000);
          switch (type)
          {
            case 0:
              j = 8220100;
            ...
            case 1:
              for (i = 0; i < 20; i++)
              {
                ...
                mapleMonster.getStats().setLevel((short) (chr.getLevel() + 10));
            ...
            case 2:
              ...
              setEliteChampionFinal(false);
              setEliteChampionCount(5);
            ...
            //   Flower.setHp(15L);
            //   Flower.getStats().setLevel((short)(chr.getLevel() + 10));
            //   spawnMonsterOnGroundBelow(Flower, this.monsterSpawn.get(Rand).getPosition());
            //   for (k = 0; k < 15; k++)
            //   {
            //     j = 8220111;
            //     Rand = Randomizer.rand(0, this.monsterSpawn.size() - 1);
            ...
            case 3:
              ...
              EliteChmp.setEliteType(4);
              EliteChmp.getStats().setLevel(chr.getLevel());
              EliteChmp.setHp((long) ((monster.getStats().getHp() * 500L) * EliteChmp.bonusHp()));
        ...
        else if (monster.getStats().getLevel() >= 100 && isSpawnPoint() && !getIsEliteBossMap() && !getIsEliteBossRewardMap() && !isEliteChampionMap() && !monster.isEliteMonster() && monster.getEliteGrade() < 0)
        {
          ...
          if (!alreadyspawn)
          {
            if (getEliteCount() >= 20)
            {
              if (!getIsEliteBossMap() && !getIsEliteBossRewardMap())
              {
                startEliteBossMap();
                ...
                Timer.MapTimer.getInstance().schedule(new Runnable()
                {
                  public void run ()
                  {
                    ...
                    eliteBoss.getEliteGradeInfo().add(new Pair<>(Integer.valueOf(EliteMonsterGradeInfo.getThirdGradeInfo().get(Integer.valueOf(rand)).getSkillid()), Integer.valueOf(EliteMonsterGradeInfo.getThirdGradeInfo().get(Integer.valueOf(rand)).getSkilllv())));
    ...
    if (monster.getId() == 8820212 || monster.getId() == 8880302 || monster.getId() == 8880342 || monster.getId() == 8880150 || monster.getId() == 8880151 || monster.getId() == 8880155 || monster.getId() == 8644650 || monster.getId() == 8644655 || monster.getId() == 8880405 || monster.getId() == 8645009 || monster.getId() == 8645039 || monster.getId() == 8880504 || monster.getId() == 8880602 || monster.getId() == 8880614 || monster.getId() == 8820101 || monster.getId() == 8820001 || monster.getId() == 8860005 || monster.getId() == 8860000 || monster.getId() == 8810018 || monster.getId() == 8810122 || monster.getId() == 8840007 || monster.getId() == 8840000 || monster.getId() == 8840014 || monster.getId() == 8800102 || monster.getId() == 8500012 || monster.getId() == 8500002 || monster.getId() == 8880200 || monster.getId() == 8870000 || monster.getId() == 8800002 || monster.getId() == 8500022 || monster.getId() == 8870100 || monster.getId() == 8910100 || monster.getId() == 8930100 || monster.getId() == 8910000 || monster.getId() == 8930000 || monster.getId() == 8850011 || monster.getId() == 8850111 || monster.getId() == 8950102 || monster.getId() == 8950002 || monster.getId() == 8880111 || monster.getId() == 8880101 || (monster.getId() >= 8900100 && monster.getId() <= 8900102) || (monster.getId() >= 8900000 && monster.getId() <= 8900002) || (monster.getId() >= 8920000 && monster.getId() <= 8920003) || (monster.getId() >= 8920100 && monster.getId() <= 8920103) || monster.getId() == 8920106 || monster.getId() == 8900103)
    {
      ...
      if (chr.getParty() != null)
      {
        ...
        for (MaplePartyCharacter chrs : chr.getParty().getMembers())
        {
          if (chrs.getPlayer() != null && chr.getMapId() == chrs.getPlayer().getMapId() && chrs.isOnline())
          {
    ...
    {
      killAllMonsters(false);
      broadcastMessage(SLFCGPacket.MakeBlind(1, 255, 240, 240, 240, 1000, 0));
    }
    ...
    int[] linkMobs = {
        9010152, 9010153, 9010154, 9010155, 9010156, 9010157, 9010158, 9010159, 9010160, 9010161, 9010162, 9010163, 9010164, 9010165, 9010166, 9010167, 9010168, 9010169, 9010170, 9010171, 9010172, 9010173, 9010174, 9010175, 9010176, 9010177, 9010178, 9010179, 9010180, 9010181
    };
    ...
    if (chr.getParty() != null && GameConstants.price.containsKey(Integer.valueOf(monster.getId())))
    {
    ...
    if (this.rune == null && !chr.getBuffedValue(80002282) && !isTown() && getAllMonster().size() > 0 && isSpawnPoint() && !GameConstants.isContentsMap(getId()) && !GameConstants.보스맵(getId()) && !GameConstants.사냥컨텐츠맵(getId()) && !GameConstants.튜토리얼(getId()) && !GameConstants.로미오줄리엣(getId()) && !GameConstants.피라미드(getId()))
    {
      ...
      Point poss = this.monsterSpawn.get(sppoint).getPosition();
  ...
  public List<MapleSpecialChair> getAllSpecialChairs ()
  {
    ...
    for (MapleMapObject mmo : this.mapobjects.get(MapleMapObjectType.SPECIAL_CHAIR).values())
    {
  ...
  public List<MapleMagicSword> getAllMagicSword ()
  {
    ...
    for (MapleMapObject mmo : this.mapobjects.get(MapleMapObjectType.MagicSword).values())
    {
  ...
  {
    ArrayList<SecondAtom> ret = new ArrayList<SecondAtom>();
    for (MapleMapObject mmo : this.mapobjects.get(MapleMapObjectType.A_SECOND_ATOM).values())
    {
  ...
  public void killAllMonsters (MapleCharacter chr)
  {
    for (MapleMapObject mapleMapObject : this.getAllMonstersThreadsafe())
    {
      MapleMonster monster = (MapleMonster) mapleMapObject;
  ...
  public void killMonsterDealy (MapleMonster mob)
  {
  ...
  public void resetReactors (final MapleClient c)
  {
    this.setReactorState((byte) 0, c);
  }

  public void setReactorState (final MapleClient c)
  {
    this.setReactorState((byte) 1, c);
  }

  public void setReactorState (final byte state, final MapleClient c)
  {
    for (final MapleMapObject obj : this.mapobjects.get(MapleMapObjectType.REACTOR).values())
    {
  ...
  public void spawnZakum (MapleCharacter chr, int x, int y)
  {
  ...
  public void spawnChaosZakum (MapleCharacter chr, int x, int y)
  {
    Point pos = new Point(x, y);
  ...
  public void spawnMonsterDelay (MapleMonster monster, int spawnType, int delay)
  {
    Timer.MapTimer.getInstance().schedule(() -> spawnMonster(monster, spawnType, false), delay);
  }

  public void spawnMonsterDelayid (int mobid, int spawnType, int delay, int x, int y)
  {
    Timer.MapTimer.getInstance().schedule(() ->
    {
  ...
  public void spawnMonster (final MapleMonster monster, final int spawnType, final boolean overwrite)
  {
    ...
    this.spawnAndAddRangedMapObject(monster, new DelayedPacketCreation()
    {
      ...
      @Override public void sendPackets (MapleClient c)
      {
        if (monster.getOwner() == -1)
        {
          c.getSession().writeAndFlush(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : (int) monster.getStats().getSummonType(), 0));
        }
        else if (monster.getOwner() == c.getPlayer().getId())
        {
          c.getSession().writeAndFlush(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : (int) monster.getStats().getSummonType(), 0));
  ...
  public void spawnDelayedAttack (final MobSkill skill, final MapleDelayedAttack mda)
  {
  ...
  public void spawnDelayedAttack (MapleMonster mob, MobSkill skill, List<MapleDelayedAttack> mda)
  {
  ...
  public void spawnMapleAtom (MapleAtom atom)
  {
  ...
  public void spawnOrb (MapleOrb orb)
  {
    spawnAndAddRangedMapObject(orb, c -> orb.sendSpawnData(c));
  }

  public void removeOrb (int playerId, MapleOrb orb)
  {
    if (orb != null)
    {
      removeMapObject(orb);
    }
    broadcastMessage(CField.removeOrb(playerId, Collections.singletonList(orb)));
  }

  public void spawnSpecialPortal (MapleCharacter chr, List<SpecialPortal> objects)
  {
  ...
  public List<SpecialPortal> SpecialPortalSize (final int ownerId)
  {
  ...
  public void removeSpecialPortal (MapleCharacter chr, SpecialPortal object)
  {
    if (object != null)
    {
    ...
    broadcastMessage(CField.removeSpecialPortal(chr.getId(), Collections.singletonList(object)));
  }

  public void removeSpecialPortal (MapleCharacter chr, List<SpecialPortal> lists)
  {
  ...
  public void spawnSecondAtom (MapleCharacter chr, List<SecondAtom> tiles, int spawnType)
  {
  ...
  public void spawnSecondAtom (MapleCharacter chr, MapleSecondAtom atom)
  {
    List<MapleSecondAtom> at = Collections.singletonList(atom);
    spawnAndAddRangedMapObject(atom, c -> c.getSession().writeAndFlush(SkillPacket.createSecondAtom(at)));
  }

  public void spawnSecondAtom (MapleCharacter chr, MapleSecondAtom atom, boolean left)
  {
    List<MapleSecondAtom> at = Collections.singletonList(atom);
  ...
  public void removeSecondAtom (MapleCharacter chr, int objectId)
  {
  ...
  public void spawnRandomPortal (MapleRandomPortal portal)
  {
  ...
  public void spawnSpiderWeb (final SpiderWeb web)
  {
  ...
  public void spawnSummon (final MapleSummon summon, final int duration)
  {
    summon.updateMap(this);
  ...
  public void spawnSpecialChair (final MapleSpecialChair ex)
  {
    spawnAndAddRangedMapObject(ex, new DelayedPacketCreation()
    {
      public void sendPackets (MapleClient c)
  ...
  public void spawnMagicWreck (final MapleMagicWreck mw)
  {
  ...
  public void RemoveMagicWreck (MapleMagicWreck mw)
  {
  ...
  public void spawnFlyingSword (final MapleFlyingSword mfs)
  {
  ...
  public void removeAllFlyingSword ()
  {
  ...
  public void setNewFlyingSwordNode (final MapleFlyingSword mfs, Point point)
  {
  ...
  public void spawnIncinerateObject (final MapleIncinerateObject mio)
  {
  ...
  public void spawnFieldAttackObj (final MapleFieldAttackObj fao)
  {
  ...
  public MapleEnergySphere getEnergySphere (int objid)
  {
  ...
  public MapleMist getMist (int ownerId, int skillId)
  {
    for (MapleMist mist : getAllMistsThreadsafe())
    {
      if (mist.getSourceSkill() != null && mist.getSourceSkill().getId() == skillId && mist.getOwnerId() == ownerId)
      {
        return mist;
      }
    }
  ...
  public void removeMist (int skillid)
  {
    for (MapleMist mist : getAllMistsThreadsafe())
    {
  ...
  public void removeMistByOwner (MapleCharacter chr, int skillid)
  {
    for (MapleMist mist : getAllMistsThreadsafe())
    {
  ...
  public void removeMist (MapleMist mist)
  {
  ...
  public void spawnMesoDrop (int meso, Point position, final MapleMapObject dropper, final MapleCharacter owner, boolean playerDrop, byte droptype, final int delay)
  {
  ...
  public void spawnMobMesoDrop (int meso, final Point position, final MapleMapObject dropper, final MapleCharacter owner, boolean playerDrop, byte droptype)
  {
    final MapleMapItem mdrop = new MapleMapItem(meso, position, dropper, owner, droptype, playerDrop);

    spawnAndAddRangedMapObject(mdrop, c -> c.getSession().writeAndFlush(CField.dropItemFromMapObject(owner.getMap(), mdrop, dropper.getTruePosition(), position, (byte) 1, (owner.getBuffedEffect(SecondaryStat.PickPocket) != null))));
  ...
  public void spawnFlyingDrop (final MapleCharacter chr, final Point startPos, final Point dropPos, Item idrop)
  {
  ...
  public void spawnMobFlyingDrop (final MapleCharacter chr, final MapleMonster mob, final Point dropPos, Item idrop)
  {
  ...
  public void spawnMobDrop (Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr, byte droptype, final int questid)
  {
  ...
  public void spawnMobPublicDrop (Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr, byte droptype, final int questid)
  {
  ...
  public void spawnItemDrop (final MapleMapObject dropper, final MapleCharacter owner, Item item, Point pos, boolean ffaDrop, boolean playerDrop)
  {
  ...
  public void startMapEffect (String msg, int itemId, boolean jukebox, int time)
  {
  ...
  public void addPlayer (MapleCharacter chr)
  {
    ...
    if (chr.getDeathCount() > 0 && chr.getEventInstance() != null)
    {
      ...
      else if (chr.getMapId() == 262031300)
      {
        ...
        chr.getClient().send(CWvsContext.onFieldSetVariable("DeathCount", String.valueOf(deathcouint)));
      }
    }
    ...
    if (eventskillid > 0)
    {
      ...
      boolean cast = eventskillid != 80003046 || chr.getKeyValue(100794, "today") < ((Calendar.getInstance().get(7) == 7 || Calendar.getInstance().get(7) == 1) ? 6000L : 3000L);
    ...
    if (!this.onFirstUserEnter.isEmpty() && isFirstUserEnter())
    {
    ...
    else if (getIsEliteBossRewardMap())
    {
  ...
  public boolean makeCarnivalReactor (int team, int num)
  {
    ...
    {
      if (guard.right.intValue() == team || guard.right.intValue() == -1)
      {
        boolean found = false;
  ...
  public void killMonsters (List<MapleMonster> mon)
  {
  ...
  public void spawnSoul (MapleMapObject dropper, MapleCharacter chr, Item item, Point pos, Item weapon)
  {
    Point droppos = calcDropPos(pos, pos);
  ...
  public void spawnMagicSword (final MapleMagicSword ms, final MapleCharacter chr, final boolean core)
  {
  ...
  public Integer getCustomTime (int skillid)
  {
    if (this.customInfo.containsKey(Integer.valueOf(skillid)))
    {
      return Integer.valueOf((int) (this.customInfo.get(Integer.valueOf(skillid)).getEndTime() - System.currentTimeMillis()));
    }
    return null;
  }
  ...
  public void makeEliteMonster (int mobid, int grade, Point pos, boolean rune, boolean eliteboss)
  {
    ...
    switch (scale)
    {
      case 0:
        ...
        eliteMonster.setEliteExpRate(50);
        break;
      case 1:
        ...
        eliteMonster.setEliteExpRate(100);
        break;
      case 2:
        ...
        eliteMonster.setEliteExpRate(200);
        break;
    }
  ...
  public boolean getIsEliteBossMap ()
  {
    return this.isEliteBossMap;
  }

  public void setEliteBossMap (boolean eliteBossMap)
  {
    this.isEliteBossMap = eliteBossMap;
  }

  public boolean getIsEliteBossRewardMap ()
  {
    return this.isEliteBossRewardMap;
  }

  public void setEliteBossRewardMap (boolean eliteBossRewardMap)
  {
    this.isEliteBossRewardMap = eliteBossRewardMap;
  }

  public int getEliteChampionCount ()
  {
    return this.eliteChampionCount;
  }

  public void setEliteChampionCount (int eliteChampionCount)
  {
    this.eliteChampionCount = eliteChampionCount;
  }

  public boolean isEliteChampionMap ()
  {
    return this.isEliteChampionMap;
  }

  public void setEliteChampionMap (boolean eliteChampionMap)
  {
    this.isEliteChampionMap = eliteChampionMap;
  }

  public int getEliteChampionType ()
  {
    return this.eliteChampionType;
  }

  public void setEliteChampionType (int eliteChampionType)
  {
    this.eliteChampionType = eliteChampionType;
  }

  public boolean isEliteChampionFinal ()
  {
    return this.eliteChampionFinal;
  }

  public void setEliteChampionFinal (boolean eliteChampionFinal)
  {
    this.eliteChampionFinal = eliteChampionFinal;
  }
  ...
  public void CreateObstacle2 (MapleMonster monster, Obstacle ob, byte type)
  {
    addMapObject(ob);
  ...
  public List<SpiderWeb> getAllSpiderWeb ()
  {
    return getWebInRange(new Point(0, 0), Double.POSITIVE_INFINITY, Collections.singletonList(MapleMapObjectType.WEB));
  }

  public List<SpiderWeb> getWebInRange (Point from, double rangeSq, List<MapleMapObjectType> MapObject_types)
  {
  ...
  private class MapleMapManagement implements Runnable
  {
    ...
    public void handleCharacters (long time)
    {
      ...
      while (chrs.hasNext())
      {
        ...
        if (chr.getSkillCustomValues().size() > 0)
        {
          ...
          {
            if (sci.getValue().canCancel(System.currentTimeMillis()))
            {
              chr.removeSkillCustomInfo(sci.getKey().intValue());
              if (sci.getKey().intValue() == 33001001)
              {
                chr.getClient().send(CField.SummonPacket.JaguarAutoAttack(false));
                continue;
              }
              if (sci.getKey().intValue() == 9110 && (chr.getMapId() / 100000 == 9530 || chr.getMapId() / 100000 == 9540))
              {
                chr.warp(951000000);
                continue;
              }
              if (sci.getKey().intValue() == 24220 && (chr.getMapId() == 450008950 || chr.getMapId() == 450008350))
              {
                chr.playerIGDead();
                continue;
              }
              if (sci.getKey().intValue() == 450001401)
              {
                if (MapleMap.this.getId() == 450001400)
                {
                  chr.getClient().send(SLFCGPacket.ErdaSpectrumArea(33, 22962, 0, 2500, -120, -128, 120, 5, Randomizer.rand(-250, 1200), 47));
    ...
    public void handleMists (long time)
    {
      ...
      while (mists.hasNext())
      {
        ...
        else if (mist.getMob() != null && MapleMap.this.getMonsterByOid(mist.getMob().getObjectId()) == null)
        {
          toRemove.add(mist);
        }
        ...
        if (mist.isMobMist() && mist.getMob() != null)
        {
    ...
    public void handleMobs (long time)
    {
      ...
      if (MapleMap.this.getEliteBossRewardType() == 2 && MapleMap.this.getIsEliteBossRewardMap() && MapleMap.this.getCustomValue(81111111) == null)
      {
      ...
      if (MapleMap.this.getEliteBossRewardType() == 3 && MapleMap.this.getIsEliteBossRewardMap() && MapleMap.this.getCustomValue(81111111) == null)
      {
        MapleMap.this.setCustomInfo(81111111, 0, Randomizer.rand(200, 1000));
      ...
      if (MapleMap.this.getId() == 450008350 || MapleMap.this.getId() == 450008950)
      {
        for (MapleCharacter chr : MapleMap.this.getAllChracater())
        {
          ...
          if (!chr.getMap().getWillPoison().isEmpty())
          {
            ...
            {
              Point pos = pos2.next();
              if (pos.x - 100 < (chr.getPosition()).x && pos.x + 100 > (chr.getPosition()).x && chr.isAlive() && chr.getBuffedEffect(SecondaryStat.NotDamaged) == null && chr.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null)
              {
      ...
      if ((monster = MapleMap.this.getMonsterById(8220028)) != null && !MapleMap.this.getIsEliteBossRewardMap())
      {
        int[] itemlist = { 2432391, 2432392, 2432393, 2432394, 2432395, 2432396, 2432397 };
        int Random = 0;
        for (int i = 0; i < 3; i++)
      ...
      if (((monster = MapleMap.this.getMonsterById(8880100)) != null || (monster = MapleMap.this.getMonsterById(8880101)) != null || (monster = MapleMap.this.getMonsterById(8880110)) != null || (monster = MapleMap.this.getMonsterById(8880111)) != null) && time - MapleMap.this.lastIncinerateTime >= 30000L)
      {
      ...
      while (lifes.hasNext())
      {
        ...
        if (life.getCustomValues().size() > 0)
        {
          ...
          for (Map.Entry<Integer, SkillCustomInfo> sci : customInfo.entrySet())
          {
            if (sci.getValue().canCancel(System.currentTimeMillis()))
            {
              life.removeCustomInfo(sci.getKey().intValue());
              if (sci.getKey().intValue() == 400011122)
              {
                life.removeCustomInfo(400011121);
                continue;
              }
              if (sci.getKey().intValue() == 24205)
              {
        ...
        {
          if (!MapleMap.this.getAllChracater().contains(cancel.getRight().getChr()) && !cancel.getRight().isMobskill())
          {
        ...
        if (life.getId() == 8870100)
        {
          ...
          {
            hardhilla.setLastSkillUsed(MobSkillFactory.getMobSkill(200, 251), System.currentTimeMillis(), (Randomizer.rand(30, 40) * 1000L));
        ...
        if (life.getId() == 8880315 || life.getId() == 8880316)
        {
          if ((life.getPosition()).y > -2300 && (life.getPosition()).y < -2020)
          {
            if ((life.getController().getPosition()).y > 0)
            {
              ...
              {
                if (achr != null && (achr.getPosition()).y < -2000)
                {
          ...
          else if ((life.getPosition()).y < 300 && (life.getPosition()).y > -500 && (life.getController().getPosition()).y < 0)
          {
            for (MapleCharacter achr : life.getMap().getAllChracater())
            {
              if (achr != null && (achr.getPosition()).y > 0)
              {
                life.switchController(achr, false);
                break;
              }
        ...
        if (life.getId() == 8880102)
        {
          ...
          if (demian != null)
          {
            ...
            for (MapleCharacter chr : MapleMap.this.getAllCharactersThreadsafe())
            {
              if (chr.getSkillCustomValue(143144) == null)
              {
                boolean damaged = chr.getTruePosition().getX() > life.getTruePosition().getX() - pix && chr.getTruePosition().getX() < life.getTruePosition().getX() + pix;
                Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
        ...
        if (life.isBuffed(MonsterStatus.MS_Burned))
        {
          ...
          {
            boolean damage = buff.getInterval() <= 1000 || time - buff.getLastPoisonTime() >= buff.getInterval();
            if (damage)
            {
    ...
    public void run ()
    {
      ...
      if (MapleMap.this.eliteTime > 0 && MapleMap.this.getIsEliteBossMap())
      {
      ...
      if (ServerConstants.Event_Blooming && MapleMap.this.getId() == 993192000)
      {
      ...
      if (ServerConstants.Event_MapleLive && MapleMap.this.getId() == 993194000)
      {
        if (((new Date()).getHours() >= 10 && (new Date()).getHours() <= 24) || (new Date()).getHours() < 2)
        {
          if ((new Date()).getMinutes() >= 30 && (new Date()).getMinutes() < 40)
      ...
      if (MapleMap.this.getCustomValues().size() > 0)
      {
        ...
        for (Iterator<Map.Entry<Integer, SkillCustomInfo>> iterator = customInfo.entrySet().iterator(); iterator.hasNext(); )
        {
          ...
          if (sci.getValue().canCancel(System.currentTimeMillis()))
          {
            MapleMap.this.removeCustomInfo(sci.getKey().intValue());
            if (MapleMap.this.getChannel() == 1)
            {
              if (sci.getKey().intValue() == 921174101)
              {
              ...
              if (sci.getKey().intValue() == 993192000)
              {
                List<Pair<String, Integer>> eff = new ArrayList<>();
                eff.add(new Pair<>("bloomingSun", Integer.valueOf(0)));
                eff.add(new Pair<>("bloomingWind", Integer.valueOf(0)));
              ...
              if (sci.getKey().intValue() == 993192500)
              {
              ...
              if (sci.getKey().intValue() == 993192600)
              {
              ...
              if (sci.getKey().intValue() == 993192601)
              {
                for (MapleCharacter chr : MapleMap.this.getAllChracater())
                {
                  chr.warp(993192501);
              ...
              if (sci.getKey().intValue() == 993192602 || sci.getKey().intValue() == 993192603 || sci.getKey().intValue() == 993192604)
              {
                if (MapleMap.this.getCharactersSize() > 0)
                {
                  String name = (sci.getKey().intValue() == 993192602) ? "trick01" : ((sci.getKey().intValue() == 993192603) ? "trick02" : "trick03");
                  List<String> str = Collections.singletonList(name);
                  List<Integer> foothold = new ArrayList<>();
                  int cooltime = (sci.getKey().intValue() == 993192602) ? 4300 : (3500);
                  switch (sci.getKey().intValue())
                  {
                    case 993192602:
                  ...
                  int endtime = (sci.getKey().intValue() == 993192602) ? 2500 : (3500);
                  ...
                  {
                    MapleMap.this.setCustomInfo(sci.getKey().intValue(), 0, endtime);
              ...
              if (sci.getKey().intValue() == 993026900)
              {
              ...
              if (sci.getKey().intValue() == 921174100)
              {
                ...
                Timer.EtcTimer.getInstance().schedule(() ->
                {
                  ...
                  {
                    chrs.getChr().addKV("BattlePVPRank", String.valueOf(i));
                    chrs.getChr().addKV("BattlePVPLevel", String.valueOf(chrs.getLevel()));
                    chrs.getChr().addKV("BattlePVPKill", String.valueOf(chrs.getKill()));
                    i++;
                  }
