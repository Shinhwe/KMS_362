    ...
    Runnable r = new Runnable()
    {
      public void run ()
      {
        if (new Date().getMinutes() % 2 == 1)
        {
          ...
          String Day = (time.getDate() < 10) ? ("0" + time.getDate()) : (String.valueOf(time.getDate()));
          int Today = Integer.parseInt(Year + Month + Day);
          String Hour = (time.getHours() < 10) ? ("0" + time.getHours()) : (String.valueOf(time.getHours()));
          String Minute = (time.getMinutes() < 10) ? ("0" + time.getMinutes()) : (String.valueOf(time.getMinutes()));
