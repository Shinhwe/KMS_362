...
{// 옹우ㅠㅇ
  ...
  private final List<Pair<MonsterStatus, MonsterStatusEffect>> stati = new CopyOnWriteArrayList<>();
  private final Map<MobSkill, Long> usedSkills = new HashMap<>();
  ...
  public final void damage (MapleCharacter from, long damage, boolean updateAttackTime, int lastSkill)
  {
    ...
    else if (getId() == 8880301 || getId() == 8880341)
    {
      ...
      if (hppercent <= 50.0D && getWillHplist().contains(Integer.valueOf(500)))
      {
        ...
        this.map.broadcastMessage(MobPacket.showBossHP(this));
        if (getCustomValue(2420701) == null && !this.isSkillForbid)
        {
      ...
      if (hppercent <= 0.3D && getWillHplist().contains(Integer.valueOf(3)))
      {
        ...
        this.map.broadcastMessage(MobPacket.showBossHP(this));
        if (getCustomValue(2420702) == null && !this.isSkillForbid)
        {
          if (this.schedule != null)
          {
            this.schedule.cancel(true);
    ...
    if (GameConstants.isUnionRaid(from.getMapId()))
    {
      switch (getId())
      {
        ...
        case 9833205:
          ...
          {
            if (monster.getOwner() == from.getId() && monster.getId() == getId() - 100 && monster.getBuff(MonsterStatus.MS_PowerImmune) == null)
            {
    ...
    if ((getId() == 9390612 || getId() == 9390610 || getId() == 9390911 || getId() == 8645066) && from.getKeyValue(200106, "golrux_in") == 1L)
    {
      from.setKeyValue(200106, "golrux_dmg", String.valueOf(from.getKeyValue(200106, "golrux_dmg") + damage));
    }
    if (getId() == 8880305)
    {
    ...
    {
      if (from.getMap() != null && from.getMap().getMonsterByOid(getSeperateSoul()) != null)
      {
    ...
    if (from.getMap().getId() == 270051100 || from.getMap().getId() == 270050100)
    {
      ...
      if (getId() >= 8820002 + (choas ? 100 : 0) && getId() <= 8820006 + (choas ? 100 : 0) && from.getMap().getMonsterById(8820014 + (choas ? 100 : 0)) != null)
      {
    ...
    if (from.getMap().getId() / 10000 == 92507)
    {
      ...
      if ((stage == 9 || stage == 13 || stage == 15 || stage == 45) && damage >= getHp() && getCustomValue0(getId()) == 0L)
      {
    ...
    if (from.getMapId() == 993192800 && ((getId() >= 9833935 && getId() <= 9833946) || (getId() >= 9833947 && getId() <= 9833958)))
    {
      ...
      if (getHp() - damage <= 0L)
      {
        ...
        {
          from.getClient().setCustomData(100795, "point", String.valueOf(Integer.parseInt(from.getClient().getCustomData(100795, "point")) + getCustomValue0(getId())));
          from.getClient().send(CField.PunchKingPacket(from, 3, getObjectId(), (int) getCustomValue0(getId())));
      ...
      else
      {
        ...
        if (totaldamage >= getStats().getHp() / 100L * 10L)
        {
          ...
          from.getClient().setCustomData(100795, "point", String.valueOf(Integer.parseInt(from.getClient().getCustomData(100795, "point")) + totaladdpoint));
          from.getClient().send(CField.PunchKingPacket(from, 3, getObjectId(), totaladdpoint));
          from.getClient().send(CField.PunchKingPacket(from, 2, Integer.parseInt(from.getClient().getCustomData(100795, "point"))));
        }
      }
      if (nextStage)
      {
        ...
        else
        {
          ...
          from.getClient().send(CField.PunchKingPacket(from, 1, stage));
        }
      }
    }
    ...
    else
    {
      if (this.sponge.get() != null && this.sponge.get().hp > 0L)
      {
        ...
        {
          this.sponge.get().hp -= getStats().getHp();
        }
        else
        {
          this.sponge.get().hp -= damage;
        }
        boolean send = true;
        if (this.sponge.get().hp <= 0L && (this.sponge.get().getId() == 8810122 || this.sponge.get().getId() == 8810018))
        {
          for (MapleMonster m : getMap().getAllMonster())
          {
            if (m.getId() != this.sponge.get().getId())
            {
  ...
  {
    if (this.eliteMonster || this.isEliteBoss)
    {
  ...
  public final void killed ()
  {
    ...
    {
      if (skill.getRight().getSchedule() != null && !skill.getRight().getSchedule().isDone())
      {
        skill.getRight().getSchedule().cancel(true);
      }
    }
  ...
  public final int killBy (MapleCharacter killer, int lastSkill)
  {
    ...
    if (killer != null && killer.getPosition() != null && (getId() < 8644101 || getId() > 8644112) && getStats().getLevel() >= killer.getLevel() - 20 && getStats().getLevel() <= killer.getLevel() + 20)
    {
      if (killer.getBuffedValue(80003025))
      {
        ...
        if (killer.getKeyValue(100722, "today") < maximum)
        {
          ...
          if (givecoin)
          {
            killer.setKeyValue(100722, "today", String.valueOf(killer.getKeyValue(100722, "today") + 1L));
            killer.setKeyValue(100722, "cnt", String.valueOf(killer.getKeyValue(100722, "cnt") + 1L));
          ...
          if (killer.getKeyValue(100711, "today") < 20000L)
          {
            if (killer.getEventMobCount() > 0)
            {
              ...
              if (killer.getEventMobCount() <= 0)
              {
                ...
                killer.setEventSkillTimer(null);
              }
            }
            else
            {
        ...
        else
        {
          ...
          if ((Randomizer.isSuccess(5) || v) && !getStats().isBoss())
          {
            if (killer.getKeyValue(100711, "today") < 30000L)
            {
              ...
              {
                killer.getClient().getSession().writeAndFlush(SLFCGPacket.SkillfromMonsterEffect(80003016, 0, (getTruePosition()).x, (getTruePosition()).y));
                killer.setKeyValue(100708, "fever", String.valueOf(killer.getKeyValue(100708, "fever") + 5L));
              }
    ...
    if (killer.getBuffedValue(80003025))
    {
      if (getId() == 9833905)
      {
        killer.setKeyValue(100722, "today", String.valueOf(killer.getKeyValue(100722, "today") + 20L));
  ...
  public final boolean isBuffed (MonsterStatus status)
  {
    ...
    while (itr.hasNext())
    {
      ...
      if (skill != null && skill.getLeft() == status)
      {
        return true;
      }
    }
  ...
  public final boolean isBuffed (int skillid)
  {
    ...
    {
      Pair<MonsterStatus, MonsterStatusEffect> skill = itr.next();
      if (skill != null && skill.getRight().getSkill() == skillid)
      {
        return true;
      }
    }
  ...
  public final MonsterStatusEffect getBuff (int skillid)
  {
    ...
    {
      Pair<MonsterStatus, MonsterStatusEffect> skill = itr.next();
      if (skill.getRight().getSkill() == skillid)
      {
        return skill.getRight();
      }
    }
  ...
  public final int getBurnedBuffSize (int skillid)
  {
    ...
    {
      Pair<MonsterStatus, MonsterStatusEffect> skill = itr.next();
      if (skill.getRight().getSkill() == skillid && skill.getLeft() == MonsterStatus.MS_Burned)
      {
        size++;
      }
    }
  ...
  public void applyStatus (MapleClient c, List<Pair<MonsterStatus, MonsterStatusEffect>> datas, SecondaryStatEffect effect)
  {
    ...
    RemoveStati(null, null, false);
    Pair<MonsterStatus, MonsterStatusEffect> re = null;
    for (Pair<MonsterStatus, MonsterStatusEffect> data : datas)
    {
      Ignition ig = null;
      int maxSuperPos = (data.getRight().getSkill() == 4120011 || data.getRight().getSkill() == 4220011 || data.getRight().getSkill() == 4340012) ? 3 : effect.getDotSuperpos();
      int dotSuperpos = 0;
      for (Ignition ign : getIgnitions())
      {
        if (ign.getSkill() == data.getRight().getSkill())
        {
          dotSuperpos++;
        }
      }
      ...
      ScheduledFuture<?> schedule = Timer.BuffTimer.getInstance().schedule(() -> action.run(), data.getRight().getDuration());
      data.getRight().setSchedule(schedule);
      data.getRight().setChr(c.getPlayer());
      data.getRight().setStati(data.getLeft());
      ...
      if (data.getLeft() == MonsterStatus.MS_Burned)
      {
        ...
        if ((effect.getDotSuperpos() == 0 && dotSuperpos <= 0) || dotSuperpos < maxSuperPos)
        {
          ...
          int du = data.getRight().getDuration();
          if (c.getPlayer().getSkillLevel(2110000) > 0)
          {
            ...
            data.getRight().setDuration(data.getRight().getDuration() * (100 + extremeMagic.getX()) / 100);
          }
          if (effect.getDotInterval() > 0 && data.getRight().getValue() > 0L)
          {
            data.getRight().setLastPoisonTime(System.currentTimeMillis());
            data.getRight().setInterval(interval);
          }
          ig = new Ignition(c.getPlayer().getId(), data.getRight().getSkill(), (value > 0L) ? value : data.getRight().getValue(), interval, du);
          data.getRight().setKey(ig.getIgnitionKey());
      ...
      else if (data.getLeft() == MonsterStatus.MS_SeperateSoulP && getStats().getCategory() != 1)
      {
        ...
        mob.applyStatus(c, MonsterStatus.MS_SeperateSoulC, data.getRight(), (int) data.getRight().getValue() + 2000, effect);
        mob.applyStatus(c, MonsterStatus.MS_Stun, data.getRight(), (int) data.getRight().getValue() + 2000, effect);
  ...
  public void applyStatus (MapleClient c, MonsterStatus status, MonsterStatusEffect effect, int value, SecondaryStatEffect eff)
  {
    ...
    ScheduledFuture<?> schedule = Timer.BuffTimer.getInstance().schedule(() -> action.run(), effect.getDuration());
  ...
  public final void cancelStatus (List<Pair<MonsterStatus, MonsterStatusEffect>> cancels)
  {
    ...
    for (Pair<MonsterStatus, MonsterStatusEffect> cancel : cancels)
    {
      ...
      ScheduledFuture<?> schedule = cancel.getRight().getSchedule();
      if (schedule != null && !schedule.isCancelled())
      {
        schedule.cancel(true);
      }
      if (cancel.getLeft().isStacked())
      {
        removeindie.add(cancel.getRight());
        getIndielist().remove(cancel.getRight());
      }
      if (cancel.getRight().getSkill() == 12101024 || cancel.getRight().getSkill() == 12121002)
      {
        this.map.broadcastMessage(CField.ignitionBomb(12100029, getObjectId(), getTruePosition()));
        continue;
      }
      if (cancel.getRight().getSkill() == 11121004)
      {
  ...
  public final void cancelStatus (MonsterStatus stat, MonsterStatusEffect effect, boolean autocancel)
  {
    ...
    {
      if (ms != null && effect != null && ms.getLeft() == stat && ms.getRight().getSkill() == effect.getSkill())
      {
        mse = ms.getRight();
        break;
      }
    ...
    {
      if (list.getRight().getSkill() == mse.getSkill())
      {
  ...
  {
    return (this.lastDropTime > 0L && this.lastDropTime + (this.stats.getDropItemPeriod() * 1000L) < now);
  }
  ...
  public void applyMonsterBuff (MapleMap map, List<Pair<MonsterStatus, MonsterStatusEffect>> stats, MobSkill mobSkill)
  {
    ...
    {
      e.getRight().setLevel(mobSkill.getSkillLevel());
      int time = 0;
      if (e.getLeft().getFlag() == MonsterStatus.MS_ExchangeAttack.getFlag() || e.getLeft().getFlag() == MonsterStatus.MS_PopulatusTimer.getFlag())
      {
        time = 60000;
      }
      CancelStatusAction action = new CancelStatusAction(this, e.getLeft(), e.getRight());
      ScheduledFuture<?> schedule = Timer.BuffTimer.getInstance().schedule(() -> action.run(), (time > 0) ? time : e.getRight().getDuration());
      e.getRight().setMobskill(true);
      if (isBuffed(e.getLeft()))
      {
        cancelStatus(e.getLeft(), e.getRight());
      }
      e.getRight().setSchedule(schedule);
  ...
  public final void RemoveStati (MonsterStatus stat, MonsterStatusEffect effect, boolean ig)
  {
    ...
    if (ig && stat != null && effect != null)
    {
      ...
      {
        Pair<MonsterStatus, MonsterStatusEffect> skill = itr.next();
        if (stat == skill.getLeft() && skill.getRight().getSkill() == effect.getSkill())
        {
          remove.add(skill);
          break;
        }
    ...
    {
      Pair<MonsterStatus, MonsterStatusEffect> skill = itr.next();
      if (System.currentTimeMillis() >= skill.getRight().getStartTime() + skill.getRight().getDuration())
      {
  ...
  public final void handleSteal (MapleCharacter chr)
  {
    ...
    if (level > 0 && !getStats().isBoss() && this.stolen == -1 && steal.getEffect(level).makeChanceResult())
    {
      ...
      List<MonsterDropEntry> de = mi.getAllDropListByMonster(this);
      if (de == null)
      {
        this.stolen = 0;
        return;
  ...
  public void setEliteMonster ()
  {
    this.eliteMonster = true;
  }

  public boolean isEliteBoss ()
  {
    return this.isEliteBoss;
  }

  public void setEliteBoss ()
  {
  ...
  public void ResetSerenTime (boolean show)
  {
    ...
    if (show)
    {
      getMap().broadcastMessage(MobPacket.BossSeren.SerenTimer(0, 360000, this.SerenNoonTotalTime, this.SerenSunSetTotalTime, this.SerenMidNightSetTotalTime, this.SerenDawnSetTotalTime));
    }
  }

  public void AddSerenTotalTimeHandler (int type, int add, int turn)
  {
    getMap().broadcastMessage(MobPacket.BossSeren.SerenTimer(1, this.SerenNoonTotalTime, this.SerenSunSetTotalTime, this.SerenMidNightSetTotalTime, this.SerenDawnSetTotalTime, turn));
  }
  ...
  public void AddSerenTimeHandler (int type, int add)
  {
    ...
    if (nowtime <= 0)
    {
      if (seren != null)
      {
        ...
        getMap().broadcastMessage(MobPacket.BossSeren.SerenTimer(2, 1));
        ...
        Timer.MapTimer.getInstance().schedule(() ->
        {
          ...
          getMap().broadcastMessage(MobPacket.BossSeren.SerenTimer(2, 0));
  ...
  private class PartyAttackerEntry implements AttackerEntry
  {
    ...
    public List<MapleMonster.AttackingMapleCharacter> getAttackers ()
    {
      ...
      {
        MapleCharacter chr = MapleMonster.this.map.getCharacterById(entry.getKey().intValue());
        if (chr != null)
        {
    ...
    private final Map<MapleCharacter, MapleMonster.OnePartyAttacker> resolveAttackers ()
    {
      ...
      {
        MapleCharacter chr = MapleMonster.this.map.getCharacterById(aentry.getKey().intValue());
        if (chr != null)
        {
    ...
    public final void killedMob (MapleMap map, long baseExp, boolean mostDamage, int lastSkill)
    {
      ...
      for (Map.Entry<MapleCharacter, MapleMonster.OnePartyAttacker> attacker : resolveAttackers().entrySet())
      {
        ...
        {
          iexp = (expMap.get(expReceiver) == null) ? 0L : expMap.get(expReceiver).exp;
          double levelMod = expReceiver.getLevel() / addedPartyLevel * 0.4D;
          iexp += (int) Math.round((((attacker.getKey().getId() == expReceiver.getId()) ? 0.6D : 0.0D) + levelMod) * innerBaseExp);
      ...
      for (Map.Entry<MapleCharacter, MapleMonster.ExpMap> expReceiver : expMap.entrySet())
      {
        ...
        MapleMonster.this.giveExpToCharacter(expReceiver.getKey(), expmap.exp, mostDamage && ((expReceiver.getKey() == highest)), expMap.size(), expmap.ptysize, expmap.Class_Bonus_EXP, expmap.Premium_Bonus_EXP, lastSkill);
      }
    }
