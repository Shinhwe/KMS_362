...
public class MapleQuestAction implements Serializable
{
  ...
  private static boolean canGetItem(QuestItem item, MapleCharacter c)
  {
    ...
    if (item.job > 0)
    {
      ...
      {
        int codec = iterator.next().intValue();
      ...
      if (!jobFound && item.jobEx > 0)
      {
        ...
        {
          int codec = iterator1.next().intValue();
  ...
  public void runStart(MapleCharacter c, Integer extSelection)
  {
    ...
    switch (this.type)
    {
      case exp:
        ...
        c.gainExp((this.intStore * GameConstants.getExpRate_Quest(c.getLevel()) * (c.getStat()).questBonus * (c.getTrait(MapleTrait.MapleTraitType.sense).getLevel() * 3L / 10 + 100) / 100), true, true, true);
        break;
      case item:
        props = new HashMap<>();
        for (QuestItem item : this.items)
        ...
        if (props.size() > 0)
        {
          selection = props.get(Integer.valueOf(Randomizer.nextInt(props.size()))).intValue();
        }
        for (QuestItem item : this.items)
        {
      ...
      case quest:
        ...
        {
          c.updateQuest(new MapleQuestStatus(MapleQuest.getInstance(q.left.intValue()), q.right.intValue()));
        }
        break;
      case skill:
        sa = new HashMap<>();
        ...
        {
          int skillid = skills.left.intValue();
          int skillLevel = skills.mid.intValue();
          int masterLevel = skills.right.intValue();
          ...
          {
            int applicableJob = iterator.next().intValue();
      ...
      case sp:
        ...
        if (this.applicableJobs.size() > 0)
        {
          ...
          for (Iterator<Integer> iterator = this.applicableJobs.iterator(); iterator.hasNext(); )
          {
            int job_val = iterator.next().intValue();
