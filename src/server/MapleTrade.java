...
public class MapleTrade
{
  ...
  public final void CompleteTrade()
  {
    if (this.exchangeItems != null)
    {
      ...
      for (Item item : itemz)
      {
        ...
        MapleInventoryManipulator.addbyItem(this.chr.get().getClient(), item, false, false);
        FileoutputUtil.log(FileoutputUtil.교환로그, "[교환 완료] " + this.chr.get().getName() + "이 " + this.partner.getChr().getName() + "의 교환 창에서 " + MapleItemInformationProvider.getInstance().getName(item.getItemId()) + "을 " + item.getQuantity() + "개 획득.");
      }
    ...
    if (this.exchangeMeso > 0L)
    {
      ...
      FileoutputUtil.log(FileoutputUtil.교환메소로그, "[교환 완료] " + this.chr.get().getName() + "이 " + this.partner.getChr().getName() + "의 교환 창에서 " + (this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso)) + "를 획득.");
    }
  ...
  public final void setMeso(long meso)
  {
    ...
    if (this.chr.get().getMeso() >= meso)
    {
      ...
      FileoutputUtil.log(FileoutputUtil.교환메소로그, "[교환 메소 올림] " + this.chr.get().getName() + "이 " + this.partner.getChr().getName() + "의 교환 창에서 " + this.meso + "를 올림.");
    }
  }
  ...
  public final void addItem(Item item)
  {
    ...
    FileoutputUtil.log(FileoutputUtil.교환로그, "[교환 아이템 올림] " + this.chr.get().getName() + "이 " + this.partner.getChr().getName() + "의 교환 창에서 " + MapleItemInformationProvider.getInstance().getName(item.getItemId()) + "을 " + item.getQuantity() + "개 올림.");
  }
  
  public final void chat(String message)
  {
    if (!CommandProcessor.processCommand(this.chr.get().getClient(), message, ServerConstants.CommandType.TRADE))
    {
      this.chr.get().getClient().getSession().writeAndFlush(PlayerShopPacket.shopChat(this.chr.get(), this.chr.get().getName(), this.chr.get().getId(), message, (this.chr.get().getTrade()).tradingslot));
      if (this.partner != null)
      {
        this.partner.getChr().getClient().getSession().writeAndFlush(PlayerShopPacket.shopChat(this.chr.get(), this.chr.get().getName(), this.chr.get().getId(), message, (this.chr.get().getTrade()).tradingslot));
      }
    }
    DBLogger.getInstance().logChat(LogType.Chat.Trade, this.chr.get().getId(), this.chr.get().getName(), message, "수신 : " + this.partner.getChr().getName());
    if (this.chr.get().getClient().isMonitored())
    {
      World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, this.chr.get().getName(), this.chr.get().getName() + " said in trade with " + this.partner.getChr().getName() + ": " + message));
    }
  ...
  public final void chatAuto(String message)
  {
    ...
    if (this.chr.get().getClient().isMonitored())
    {
      World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, this.chr.get().getName(), this.chr.get().getName() + " said in trade [Automated] with " + this.partner.getChr().getName() + ": " + message));
    }
    else if (this.partner != null && this.partner.getChr() != null && this.partner.getChr().getClient().isMonitored())
    {
      World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, this.chr.get().getName(), this.chr.get().getName() + " said in trade [Automated] with " + this.partner.getChr().getName() + ": " + message));
