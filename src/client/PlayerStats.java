...
public class PlayerStats implements Serializable
{
  ...
  private final Map<String, Integer> ApplyStatFinalDamage = new HashMap<String, Integer>();
  private final List<Integer> FinalDamage = new ArrayList<Integer>();
  private final EnumMap<Element, Integer> elemBoosts = new EnumMap(Element.class);
  private final transient Map<Integer, Integer> demonForce = new HashMap<Integer, Integer>();
  ...
  private final List<Triple<Integer, String, Integer>> psdSkills = new ArrayList<Triple<Integer, String, Integer>>();
  ...
  public final boolean setHp (long newhp, boolean silent, MapleCharacter chra, boolean igskill)
  {
      ...
      if (oldHp > this.hp && !chra.isAlive())
      {
        if (!igskill)
        {
          ...
          else if (chra.getBuffedEffect(SecondaryStat.ReviveOnce) != null)
          {
            ...
            {
              chra.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chra, chra.getBuffSource(SecondaryStat.ReviveOnce), chra.getBuffSource(SecondaryStat.ReviveOnce), 1, 0, 0, (byte) 0, true, null, null, null));
            ...
            {
              chra.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chra, 3000, 14110030, 62, 0, 0, (byte) (chra.isFacingLeft() ? 1 : 0), true, chra.getTruePosition(), null, null));
    ...
    if (GameConstants.isDemonAvenger(chra.getJob()))
    {
      ...
      chra.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, null, chra));
    }
  ...
  public void recalcLocalStats (final boolean first_login, final MapleCharacter chra)
  {
    ...
    for (final Map.Entry<Integer, Integer> entry : this.setHandling.entrySet())
    {
      ...
      if (set2 != null)
      {
        ...
        boolean zeroweaponlucky = set2.setItemID == chra.getKeyValue(46523, "luckyscroll") && set2.zeroWeaponJokerPossible;
        for (final Map.Entry<Integer, StructSetItem.SetItem> ent : itemz.entrySet())
        {
          if (ent.getKey() <= entry.getValue() + (zeroweaponlucky ? 1 : 0))
          {
      ...
      for (MapleUnion union : chra.getUnions().getUnions())
      {
        ...
        if (UnionHandler.cardSkills.containsKey(union.getJob() / 10) && (jobSkillEffect = SkillFactory.getSkill(jobSkill = UnionHandler.cardSkills.get(union.getJob() / 10).intValue()).getEffect(level)) != null)
        {
          switch (jobSkill)
          {
            ...
            case 71000511:
            {
              this.fixHp += jobSkillEffect.getHpFX();
              break;
            }
            case 71000351:
            {
            ...
            case 71000711:
            {
              this.expBuffZero += jobSkillEffect.getEXPRate();
            }
          }
        }
        ...
        for (Map.Entry entry : skills.entrySet())
        {
          ...
          if (chra.getKeyValue(18791, String.valueOf(check)) != -1L && chra.getKeyValue(18791, String.valueOf(check)) != (long) check)
          {
            int skillid = (int) (71004000L + chra.getKeyValue(18791, String.valueOf(check)));
  ...
  private Pair<Long, Long> handlePassiveSkills (MapleCharacter chra)
  {
    ...
    if (chra.getSkillLevel(80000413) > 0)
    {
      ...
      if (bof > 0)
      {
        this.dam_r += bx.getEffect(bof).getDAMRate();
      }
    }
    ...
    for (int skill = 0; skill < n; ++skill)
    {
      ...
      this.localint_ += bx.getEffect(bof).getStrX();
      this.localluk += bx.getEffect(bof).getStrX();
      localmaxhp_ += bx.getEffect(bof).getMhpX();
      localmaxmp_ += bx.getEffect(bof).getMhpX();
      this.ignoreMobpdpR.add(Integer.valueOf(bx.getEffect(bof).getIgnoreMob()));
    ...
    for (int j = 0; j < skill2; ++j)
    {
      ...
      switch (skill3)
      {
        ...
        case 80000050:
        {
          this.DamagePercent += eff.getDAMRate();
          continue block73;
        }
        case 80000001:
        {
    ...
    switch (chra.getJob())
    {
      ...
      case 232:
      {
        ...
        if (bof > 0)
        {
          this.DamagePercent += bx.getEffect(bof).getDAMRate();
        }
        ...
        {
          this.DamagePercent += bx.getEffect(bof).getZ();
        }
      ...
      case 2218:
      {
        ...
        if (bof > 0)
        {
          ...
          localmaxmp_ += eff.getLv2mmp() * chra.getLevel();
          this.magic += eff.getMagicX();
        }
      ...
      case 3512:
      {
        ...
        {
          eff = bx.getEffect(bof);
          this.DamagePercent += eff.getIndieDamR();
        }
        ...
        if (bof > 0)
        {
          ...
          this.DamagePercent += size * eff.getW();
        }
      ...
      case 6112:
      {
        ...
        if (bof > 0)
        {
          ...
          this.DamagePercent += eff.getY() * up6;
        }
      ...
      case 13100:
      {
        ...
        if (bof > 0)
        {
          ...
          this.ignoreMobpdpR.add(Integer.valueOf(eff.getIgnoreMob()));
          this.dropBuff += eff.getDropR();
          this.mesoBuff += eff.getMesoRate();
          this.expBuff += eff.getEXPRate();
        }
  ...
  private Pair<Long, Long> handleBuffStats (MapleCharacter chra)
  {
    ...
    {
      String buffname;
      switch (buffname = String.valueOf(buffs.getLeft()))
      {
        case "EnrageCrDamMin":
        {
        ...
        case "ElementalCharge":
        {
          ...
          if (chra.getSkillLevel(1220010) > 0)
          {
            ...
            this.DamagePercent += chra.getElementalCharge() * advcharge.getX();
          }
        ...
        case "ItemUpByItem":
        {
          this.dropBuff += buffs.getRight().value;
          break;
        }
        case "WealthOfUnion":
        case "MesoUpByItem":
        case "MesoUp":
        {
          if (buffname.equals("MesoUp"))
          {
            ...
            this.mesoBuff += buffs.getRight().value;
            break;
          }
          this.mesoBuff += buffs.getRight().value;
          break;
        ...
        case "ExpBuffRate":
        {
          this.expBuff += buffs.getRight().value;
          break;
        }
        case "MesoGuard":
        {
          this.MesoGuardMeso += buffs.getRight().value;
          break;
        }
        case "MaxLevelBuff":
        {
        ...
        case "IndieMp":
        {
          localmaxmp_ += buffs.getRight().value;
          this.before_maxmp += buffs.getRight().value;
          break;
        }
        case "IndieHp":
        {
          localmaxhp_ += buffs.getRight().value;
          break;
        }
        case "EnhancedMaxMp":
        {
        ...
        {
          this.WatkPercent += buffs.getRight().value;
          break;
        }
        case "HowlingParty":
        {
          this.WatkPercent += buffs.getRight().value;
          this.MatkPercent += buffs.getRight().value;
          break;
        }
        case "IndieBooster":
        case "PartyBooster":
        ...
        case "IndieDamR":
        {
          this.DamagePercent += buffs.getRight().value;
          break;
        }
        case "MHPCutR":
        {
        ...
        case "ArcaneAim":
        {
          this.DamagePercent += chra.getBuffedEffect(buffs.getLeft()).getX() * chra.getArcaneAim();
          break;
        }
        case "Infinity":
        {
  ...
  public final void relocHeal (MapleCharacter chra)
  {
    ...
    else if (playerjob == 1111 || playerjob == 1112)
    {
      ...
      if (lvl3 > 0)
      {
        this.shouldHealMP += effect.getEffect(lvl3).getMp();
        this.mpRecoverTime = 4000;
      }
    }
      ...
      for (final Triple<String, String, String> add : additions)
      {
        ...
        switch (s)
        {
          ...
          case "critical":
          {
            ...
            if ((job == null || canJob) && (level == null || canLevel))
            {
  ...
  public int BeforeStatWatk (MapleCharacter chr)
  {
    ...
    if (GameConstants.isKinesis(job) && (mas += a = chr.getCombo() / 30) > 99.0)
    {
      mas = 99.0;
    }
  ...
  public Pair<Long, Long> handleInnerSkills (MapleCharacter chra)
  {
    ...
    for (InnerSkillValueHolder ISVH : chra.getInnerSkills())
    {
      ...
      switch (ISVH.getSkillId())
      {
        ...
        case 70000008:
        {
          localmaxhp_ += SkillFactory.getSkill(ISVH.getSkillId()).getEffect(x).getMhpX();
          break;
        }
        case 70000009:
        {
          localmaxmp_ += SkillFactory.getSkill(ISVH.getSkillId()).getEffect(x).getMaxMpX();
          break;
        }
        case 70000010:
        {
        ...
        case 70000033:
        {
          this.mesoBuff += SkillFactory.getSkill(ISVH.getSkillId()).getEffect(x).getMesoRate();
          break;
        }
        case 70000034:
        {
        ...
        case 70000049:
        {
          this.dropBuff += SkillFactory.getSkill(ISVH.getSkillId()).getEffect(x).getDropR();
          break;
        }
        case 70000050:
        {
