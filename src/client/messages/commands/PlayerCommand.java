...
public class PlayerCommand
{
  ...
  {
    private static final int statLim = 32767;
  ...
  public static class 몬스터 extends CommandExecute
  {
    public int execute(MapleClient c, String[] splitted)
    {
      ...
      for (MapleMapObject monstermo : c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getPosition(), 100000.0D, Collections.singletonList(MapleMapObjectType.MONSTER)))
      {
        mob = (MapleMonster) monstermo;
        if (mob.isAlive())
        {
          c.getPlayer().dropMessage(6, "몬스터 정보 :  " + mob);
          break;
        }
      }
      if (mob == null)
  ...
  {
    private static final int[] npcs = new int[]{9000162, 9000000, 9010000};
    protected int npc = -1;
  ...
  public static class 메획 extends OpenNPCCommand
  {
    ...
    public int execute(MapleClient c, String[] splitted)
    {
      String String = "메소 획득량 정보 (최대 300.0%) : 현재 획득량 " +
          c.getPlayer().getStat().mesoBuff +
          "%                        기본 100.0% 임";
      c.getPlayer().dropMessage(5, String);
      return 1;
    }
  }
  ...
  public static class 인벤초기화 extends CommandExecute
  {
    public int execute(MapleClient c, String[] splitted)
    {
      ...
      {
        MapleInventoryManipulator.removeFromSlot(c, eq.getValue(), eq.getKey().left.shortValue(), eq.getKey().right.shortValue(), false, false);
      }
      return 1;
    }
  }
