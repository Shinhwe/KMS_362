...
public class MobPacket
{
  ...
  public static byte[] moveMonster (byte bOption, int skill, long targetInfo, int tEncodedGatherDuration, int oid, Point startPos, Point startWobble, List<LifeMovementFragment> moves, List<Point> multiTargetForBall, List<Short> randTimeForAreaAttack, boolean cannotUseSkill)
  {
    ...
    {
      mplew.writeShort(randTimeForAreaAttack.get(i).shortValue());
    }
    mplew.writeInt(0);
  ...
  public static byte[] spawnMonster (MapleMonster life, int spawnType, int link, int control)
  {
    ...
    /*  424 */
    boolean flying = life.getStats().getName().contains("자폭");
    /*  425 */
    /*  426 */
    /*  428 */
    if (life.getStats().getName().equals("어둠의 집행자"))
    {/*  430 */
      flying = true;
    }
    /*  433 */
    ...
    if (life.getEliteGrade() >= 0)
    {
      ...
      {
        /*  459 */
        mplew.writeInt(info.left.intValue());
        /*  460 */
        mplew.writeInt(info.right.intValue());
      }
      /*  462 */
    ...
    for (Triple<String, Integer, Integer> skeleton : life.getStats().getSkeleton())
    {
      ...
      /*  518 */
      mplew.write(skeleton.mid.intValue());
      /*  519 */
      mplew.writeInt(skeleton.right.intValue());
    }
    /*  521 */
    mplew.write(0);
    /*  522 */
  ...
  private static void encodeTemporary (MaplePacketLittleEndianWriter mplew, MapleMonster mob, List<Pair<MonsterStatus, MonsterStatusEffect>> mse, SecondaryStatEffect effect)
  {
    ...
    {
      if (ms.getLeft().isStacked() && !checked.contains(ms.getLeft()))
      {
    ...
    for (Pair<MonsterStatus, MonsterStatusEffect> ms : newstatups)
    {
      ...
      if (!ms.getRight().isMobskill() && mobStat.getFlag() <= MonsterStatus.MS_PopulatusInvincible.getFlag() && !ms.getLeft().isStacked() && !checked.contains(ms.getLeft()))
      {
        mplew.writeInt(ms.getRight().getValue());
        mplew.writeInt(ms.getRight().getSkill());
        mplew.writeShort(ms.getRight().getDuration() / 1000);
        checked.add(ms.getLeft());
      }
    }
    for (Pair<MonsterStatus, MonsterStatusEffect> ms : newstatups)
    {
      ...
      {
        mplew.writeInt(ms.getRight().getValue());
        mplew.writeShort(ms.getRight().getSkill());
        mplew.writeShort(ms.getRight().getLevel());
        if (ms.getLeft().getFlag() == MonsterStatus.MS_PopulatusTimer.getFlag())
        {
          mplew.writeShort(120);
          mplew.writeInt((mob.getCustomTime(2412).intValue() != 0) ? mob.getCustomTime(2412).intValue() : ms.getRight().getDuration());
        }
        else
        {
          mplew.writeShort(ms.getRight().getDuration() / 1000);
    ...
    if (getMse(mse, MonsterStatus.MS_Incizing))
    {
      ...
      mplew.writeInt(10); // 猜測: 隊友提升的爆傷數值
      mplew.writeInt(0); // 沒有任何用處, 可能是結束符
    }
    ...
    if (getMse(mse, MonsterStatus.刺傷))
    {
      MonsterStatusEffect eff = null;
      for (Pair<MonsterStatus, MonsterStatusEffect> stat : mse)
      {
        if (stat.getLeft() == MonsterStatus.刺傷)
        {
          eff = stat.getRight();
          break;
        }
      ...
      if (eff != null)
      {
        int effectSkillLevel = eff.getEffectSkillLevel();
        int AttackReduce = SkillFactory.getSkill(1111003).getEffect(effectSkillLevel).getW() * -1;
        mplew.writeInt(AttackReduce); // 攻擊力降低
        mplew.writeInt(0); // 不知道是什麼東西
      }
      else
      {
        mplew.writeInt(0); // 攻擊力降低
        mplew.writeInt(0); // 不知道是什麼東西
      }
    }
  ...
  public static byte[] cancelMonsterStatus (MapleMonster mons, List<Pair<MonsterStatus, MonsterStatusEffect>> mse, List<MonsterStatusEffect> indielist)
  {
    ...
    if (getMse(mse, MonsterStatus.MS_Burned))
    {
      ...
      for (Pair<MonsterStatus, MonsterStatusEffect> stat : mse)
      {
        mplew.writeInt(stat.getRight().getChr().getId());
        mplew.writeInt(stat.getRight().getSkill());
        mplew.writeInt(0);
      }
      mplew.write(5);
    }
  ...
  public static byte[] jinHillahBlackHand (int objectId, int skillId, int skillLv, List<Triple<Point, Integer, List<Rectangle>>> points)
  {
    ...
    for (Triple<Point, Integer, List<Rectangle>> point : points)
    {
      ...
      mplew.writeInt(point.mid.intValue());
      mplew.writeInt(0);
      mplew.writeInt(0);
      ...
      switch (level)
      {
        ...
        case 14:
        {
          ...
          for (Triple<Integer, Integer, Integer> value : values)
          {
            ...
            mplew.writeInt(value.mid);
            mplew.writeInt(value.right);
            mplew.writeInt(0);
          }
          break;
        }
