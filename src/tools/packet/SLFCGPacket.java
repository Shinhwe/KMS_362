...
//  }
  ...
  public static byte[] DreamBreakerRanking(String name)
  {
    ...
    packet.writeInt(DreamBreakerRank.Rank.containsKey(name) ? DreamBreakerRank.Rank.get(name).intValue() : 0);
    ...
    for (Map.Entry<String, Integer> info : DreamBreakerRank.Rank.entrySet())
    {
      ...
      packet.writeZeroBytes(8);
      packet.writeInt(info.getValue().intValue());
  ...
  public static byte[] EventInfoPut(List<Triple<Integer, Integer, String>> info)
  {
    ...
    {
      mplew.writeInt(-1);
      mplew.writeInt(linfo.getLeft().intValue());
      mplew.writeInt(linfo.getMid().intValue());
  ...
  public static class OneCardGamePacket
  {
    ...
    public static byte[] CreateUI(MapleCharacter chr, int position, List<MapleCharacter> chrs)
    {
      ...
      {
        mplew.write(a);
        AvatarLook.encodeAvatarLook(mplew, chrs.get(a), (chr.getId() == chrs.get(a).getId()), false);
      ...
      for (MultiYutGame.MultiYutPlayer player : players)
      {
        ...
        for (MultiYutGame.PlayersHorses horse : player.getHorses())
        {
          ...
          if (horse.isOverlapOwner() && horse.getOverlap().size() > 0)
          {
            ...
            {
              mplew.writeInt(OverlapHorseInvPos.getKey().intValue());
            }
          }
        }
        for (Map.Entry<Integer, Integer> Yut : player.getYut().entrySet())
        {
          mplew.writeInt(Yut.getValue().intValue());
        }
      }
      mplew.write(4);
      mplew.writeInt(2);
  ...
  public static class MonsterPyramidPacket
  {
    public static byte[] createUI(List<MapleCharacter> list)
    {
      ...
      for (int i = 0; i < list.size(); i++)
      {
        ...
        maplePacketLittleEndianWriter.write(false);
        maplePacketLittleEndianWriter.writeInt(i);
        maplePacketLittleEndianWriter.writeInt(list.get(i).getId());
        maplePacketLittleEndianWriter.writeInt(0);
    ...
    public static byte[] onInit(List<MapleCharacter> list)
    {
      ...
      for (int i = 0; i < list.size(); i++)
      {
        ...
        for (int j = 0; j < 6; j++)
        {
          maplePacketLittleEndianWriter.writeInt(list.get(i).getMonsterPyramidInstance().getPlayer(list.get(i)).getSelectBlockType(j));
        }
      }
      maplePacketLittleEndianWriter.write(0);
      return maplePacketLittleEndianWriter.getPacket();
      ...
      switch (args[1])
      {
        case 3:
          if (!first)
          {
            for (int j = 0; j < list.size(); j++)
            {
              ...
              mplew.write(false);
              mplew.writeInt(j);
              mplew.writeInt(list.get(j).getId());
              mplew.writeInt(list.get(j).getMonsterPyramidInstance().getPlayer(list.get(j)).getPoint());
              mplew.writeInt(list.get(j).getMonsterPyramidInstance().getPlayer(list.get(j)).getRank());
              for (int m = 0; m < 6; m++)
              {
                mplew.writeInt(list.get(j).getMonsterPyramidInstance().getPlayer(list.get(j)).getSelectBlockType(m));
              }
            }
            mplew.write(3);
            mplew.writeInt(args[4]);
        ...
        case 5:
          for (i = 0; i < list.size(); i++)
          {
            ...
            mplew.write(false);
            mplew.writeInt(i);
            mplew.writeInt(list.get(i).getId());
            mplew.writeInt(list.get(i).getMonsterPyramidInstance().getPlayer(list.get(i)).getPoint());
            mplew.writeInt(list.get(i).getMonsterPyramidInstance().getPlayer(list.get(i)).getRank());
            for (int j = 0; j < 6; j++)
            {
              mplew.writeInt(list.get(i).getMonsterPyramidInstance().getPlayer(list.get(i)).getSelectBlockType(j));
            }
          }
          for (k = 0; k < 7; k++)
          {
  ...
  public static class MultiOthelloGamePacket
  {
    public static byte[] createUI(List<MapleCharacter> list, MapleCharacter mapleCharacter, int i)
    {
      ...
      for (i = 0; i < list.size(); i++)
      {
        maplePacketLittleEndianWriter.write(list.get(i).getBattleReverseInstance().getPlayer(list.get(i).getId()).getStoneId());
        AvatarLook.encodeAvatarLook(maplePacketLittleEndianWriter, list.get(i), (mapleCharacter.getId() == list.get(i).getId()), false);
        maplePacketLittleEndianWriter.writeMapleAsciiString(list.get(i).getName());
        maplePacketLittleEndianWriter.writeShort(list.get(i).getJob());
      }
      maplePacketLittleEndianWriter.write(-1);
      return maplePacketLittleEndianWriter.getPacket();
    }
  ...
  public static class ColorCardPacket
  {
    ...
    public static byte[] ColorCardSetting()
    {
      ...
      {
        mplew.writeInt(alist.getLeft().intValue());
        mplew.writeInt(alist.getRight().intValue());
      }
      return mplew.getPacket();
    }
    ...
    public static byte[] ColorCardSetting1()
    {
      ...
      {
        mplew.writeInt(alist.getLeft().intValue());
        mplew.writeInt(alist.getMid().intValue());
        mplew.write(alist.getRight().intValue());
      }
      return mplew.getPacket();
    }
  ...
  public static class BigWispPacket
  {
    ...
    public static byte[] BigWispRank(Map<MapleCharacter, Integer> info)
    {
      ...
      {
        mplew.writeInt(i);
        mplew.writeInt(linfo.getKey().getId());
        mplew.writeMapleAsciiString(linfo.getKey().getName());
        mplew.writeInt(linfo.getValue().intValue());
        i++;
      }
      return mplew.getPacket();
    }
