...
public class CSPacket
{
  public static byte[] warpCS(MapleClient c)
  {
    ...
    {
      packet.writeInt(items.getKey().intValue());
      if (items.getKey().intValue() / 1000 == 5533)
      {
        packet.writeInt(items.getValue().size());
    ...
    {
      packet.write(item.left.intValue());
      packet.writeInt(item.right.intValue());
    }
    packet.writeInt(0);
    for (i = 0; i < 0; i++)
    {
  ...
  public static void addModCashItemInfo(MaplePacketLittleEndianWriter mplew, CashItemInfo.CashModInfo item)
  {
    ...
    if ((flags & 0x10000) != 0)
    {
      ...
      else
      {
        ...
        {
          mplew.writeInt(pack.get(i).intValue());
        }
      }
    }
    if ((flags & 0x80000) != 0)
  ...
  public static byte[] showBoughtCSPackage(Map<Integer, Item> ccc, MapleClient c)
  {
    ...
    {
      addCashItemInfo(mplew, sn.getValue(), c, sn.getKey().intValue(), 0, 0);
    }
    mplew.writeShort(0);
    mplew.writeInt(0);
    mplew.write(false);
  ...
  public static byte[] showCouponRedeemedItem(Map<Integer, Item> items, int mesos, int maplePoints, MapleClient c)
  {
    ...
    {
      addCashItemInfo(mplew, item.getValue(), c, item.getKey().intValue(), 0, 0);
    }
