  ...
  public final void startHairRoom(MapleClient c, int npc, String script, byte result, int slot, byte temp)
  {
    try
    {
      if (!this.cms.containsKey(c) && c.canClickNPC())
      {
        ...
        try
        {
          iv.invokeFunction("start", Byte.valueOf(result), Integer.valueOf(slot), Byte.valueOf(temp));
        }
  ...
  public final void zeroaction(MapleClient c, byte mode, byte type, int selection1, int selection2)
  {
    if (mode != -1)
    {
      ...
      try
      {
        ...
        {
          c.setClickedNPC();
          cm.getIv().invokeFunction("zeroaction", Byte.valueOf(mode), Byte.valueOf(type), Integer.valueOf(selection1), Integer.valueOf(selection2));
        }
      }
      catch (Exception e)
      {
