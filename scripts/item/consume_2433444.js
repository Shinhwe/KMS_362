...
function action(M, T, S) {
  ...
  if (St == 0) {
    ...
    if (
      cm.getPlayer().getKeyValue(1912211, "timerf") == -1 &&
      cm.getPlayer().getLevel() <= 20
    ) {
      cm.sendOkS(
        "#b#h ##k恭喜, 成功領取下列裝備!\r\n\r\n#b" +
          "#fUI/UIWindow2.img/QuestIcon/4/0#\r\n" +
  ...
  function addOption(i, isWeapon) {
    item =
      Packages.server.MapleItemInformationProvider.getInstance().generateEquipById(
        i,
        -1,
        false
      );
