	...
	if (status == 0) {
		...
		for (i = 0; i < types.length; i++) {
			...
			if (i == 0) {
				...
				if (image > 0) {
					msg += "#fs14##fc0xFF6600CC##i" + image + "#  \t" + name + " #r(착용 중)\r\n\r\n";
				} else {
		...
		inven = cm.getPlayer().getCustomInventory();
		var msg = "#fs14##b장비 목록을 선택해주세요.\r\n";
		for (i = 0; i < types.length; i++) {
			type = types[i];
			name = type.name();
			if (i > 0) {
				msg += "#fs14##fc0xFF6600CC##L" + i + "# " + name + "\r\n";
			}
		}
		cm.sendSimple(msg);
	} else if (status == 2) {
		...
		var msg = "#fs14##fc0xFF000000#보유하고 있는 #fc0xFF6600CC#" + name + " #fc0xFF000000#목록 #b(클릭 시 장착 및 장착해제)#fc0xFF000000#\r\n";
