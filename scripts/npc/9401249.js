    ...
    if (status == 0) {
        ...
        if (selection == 0) { // 스킨
            ...
            if (cm.getPlayer().getHair() >= 39600 && cm.getPlayer().getHair() <= 39997) {
                cm.sendOk("#fs14##b스페셜 헤어#k 상태에서는 믹스염색이 불가능하답니다.");
                cm.dispose();
                return;
            }
            st = selection;
            ...
            if (cm.getPlayer().getJob() == 10112 || cm.getPlayer().getJob() == 6001 || (cm.getPlayer().getJob() >= 6500 && cm.getPlayer().getJob() <= 6512)) {
                cm.sendNext("#fs14##r현재 직업군은 사용할 수 없는 기능입니다.");
                cm.dispose();
                return;
            }
            말 = "#fs14# #d남자 캐릭터#k로 변경 시,\r\n"
        ...
        if (st == 7) {
            ...
            if (cm.getPlayer().getJob() == 10112 || cm.getPlayer().getJob() == 6001 || (cm.getPlayer().getJob() >= 6500 && cm.getPlayer().getJob() <= 6512)) {
                cm.sendNext("#fs14#현재 직업군은 사용할 수 없는 기능입니다.");
                cm.dispose();
                return;
            }
            ...
            cm.sendOk("#fs14##b성별#k이 정상적으로 변경 되었습니다.");
            cm.dispose();
        } else {
