    ...
    if (status == 0) {
        ...
        if (selection == 0) {
            if (cm.getPlayer().getKeyValue(20, "talak1") == 1) {
                text += "#fs14#이미 #b" + week + "요일#k의 일일 퀘스트를 완료하였습니다."
                cm.sendOk(text);
                cm.dispose();
                return;
            } else {
                ...
                for (i = 0; i < reqitem.length; i++) {
                    text += "#fs14##i" + reqitem[i] + "# #z" + reqitem[i] + "# " + reqitemea[i] + "개\r\n"
                    if (i == (reqitem.length - 1)) {
                        text += "\r\n"
                    }
                }
                if (cm.getPlayer().getKeyValue(20, "talak1") == 2) {
                    text += "#fs14##k입니다. 지금 완료하시겠습니까?"
                } else {
                    text += "#fs14##k입니다. 지금 시작하시겠습니까?"
                }
                cm.sendYesNo(text);
            }
        ...
        if (cm.getPlayer().getKeyValue(20, "talak1") == 2) {
            for (i = 0; i < reqitem.length; i++) {
                if (cm.getPlayer().getItemQuantity(reqitem[i], false) < reqitemea[i]) {
                    ready = false;
                    text += "#fs14##t" + reqitem[i] + "##n\r\n"
                    text += "#fs14##r" + cm.getPlayer().getItemQuantity(reqitem[i], false) + "#k / " + reqitemea[i] + "개\r\n\r\n"
                }
            }
            if (ready == false) {
                text += "#fs14#퀘스트를 완료하기 위한 조건이 부족합니다.\r\n"
            } else {
                cm.getPlayer().setKeyValue(20, "talak1", 1);
                text += "#fs14#퀘스트를 완료하였습니다.\r\n\r\n"
