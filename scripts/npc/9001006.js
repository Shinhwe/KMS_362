...
function action(mode, type, selection) {
    ...
    if (mode == 0 && status == 2) {
       cm.sendNext("#fs14#흥!..너가 내마음을 모르는구나!"); // 퀘스트 수락 여부 물을 때 거절할 시 메세지
       cm.dispose();
       return;
    }
    if (mode == 0) {
   ...
   // 여기까지 퀘스트 선언, 건들지말 것
	...
	if (qstatus != 2) {
		...
		ms += "#fs14##b#L0#푸치의 고민#l\r\n" // 퀘스트 제목 이름
		if (qstatus != 0) {
			ms += "\r\n\r\n"
			ms += icon_etc + "\r\n"
			ms += "#fs14##b#L1#퀘스트를 포기하겠습니다.#l\r\n" // 퀘스트 포기 이름
		}
		cm.sendNext(ms);
	} else {
		cm.sendNext("#fs14#이미 이 퀘스트를 수행하였습니다."); // 퀘스트 이미 완료 했을 시 메세지
		cm.dispose();
	}
    } else if (status == 1) {
	sel = selection;
