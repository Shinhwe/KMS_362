...
function action(mode, type, selection) {
	...
	if (status == 0) {
		말 = "#L4##fn나눔고딕##fs14##fc0xFFEDA900#" + 스타포스 + " 적정 레벨 사냥터#k로 이동하겠습니다.#l\r\n"
		...
		if (selection == 0) { //사냥터
			...
			if (cm.getPlayer().getLevel() > 210) {
				말 += "#fs14#"+검정+"210레벨을 넘어 더 이상 추천 사냥터가 없습니다.\r\n"
				cm.sendOkS(말, 0x04, 9401243);
				cm.dispose();
			}
			if (cm.getPlayer().getLevel() < 210) {
				말 += "#fs14#"+검정+"현재 #b#h ##k"+검정+"님의 레벨은 "+cm.getPlayer().getLevel()+" 입니다.\r\n아래는 현재 레벨에 맞는 #r추천 사냥터 목록#k "+검정+"입니다.\r\n"
			}
		...
		if (selection == 1000) {
			말 = "#fs14##dA. 아케인심볼은 다음과 같은곳에서 획득 가능합니다.\r\n\r\n";
			...
			if (selectedMap <= 2) {
				if (cm.getPlayer().getLevel() < 200) {
					cm.sendOkS("#fs14##r레벨 200 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 6) { // 리버스
				if (cm.getPlayer().getLevel() < 205) {
					cm.sendOkS("#fs14##r레벨 205 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 10) { //츄츄
				if (cm.getPlayer().getLevel() < 210) {
					cm.sendOkS("#fs14##r레벨 210 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 13) { //얌얌
				if (cm.getPlayer().getLevel() < 215) {
					cm.sendOkS("#fs14##r레벨 215 달성 시 입장이 가능합니다..", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 17) { //레헬른
				if (cm.getPlayer().getLevel() < 220) {
					cm.sendOkS("#fs14##r레벨 220 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 22) { //아르카나
				if (cm.getPlayer().getLevel() < 225) {
					cm.sendOkS("#fs14##r레벨 225 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 28) { //모라스
				if (cm.getPlayer().getLevel() < 230) {
					cm.sendOkS("#fs14##r레벨 230 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 31) { //에스페라
				if (cm.getPlayer().getLevel() < 235) {
					cm.sendOkS("#fs14##r레벨 235 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 34) { //셀라스
				if (cm.getPlayer().getLevel() < 240) {
					cm.sendOkS("#fs14##r레벨 240 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 37) { //문브릿지
				if (cm.getPlayer().getLevel() < 245) {
					cm.sendOkS("#fs14##r레벨 245 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 40) { //테네브리스
				if (cm.getPlayer().getLevel() < 250) {
					cm.sendOkS("#fs14##r레벨 250 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 44) { //리멘
				if (cm.getPlayer().getLevel() < 255) {
					cm.sendOkS("#fs14##r레벨 255 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			}
	...
	*/
			...
			if (selectedMap <= 6) { // 세르니움,불타는 세르니움
				if (cm.getPlayer().getLevel() < 260) {
					cm.sendOkS("#fs14##r레벨 260 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			} else if (selectedMap <= 9) { // 호텔 아르크스
				if (cm.getPlayer().getLevel() < 265) {
					cm.sendOkS("#fs14##r레벨 265 달성 시 입장이 가능합니다.", 0x04, 9401243);
					cm.dispose();
					return;
				}
			}
