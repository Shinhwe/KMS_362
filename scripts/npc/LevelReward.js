...
function action(mode, type, sel) {
	...
	if (status == 0) {
		var msg = "#fs14# #d수령 가능한 레벨 보상은 아래와 같습니다.#k"+enter;
		...
		if (seld > 200) {
			if (getk(seld) > -1) {
				cm.sendOk("#fs14##r이미 수령하신 보상입니다.");
				cm.dispose();
				return;
			}
			if (seld > cm.getPlayer().getLevel()) {
				cm.sendOk("#fs14##r레벨이 부족합니다.#k");
				cm.dispose();
				return;
			}
			if (seld == 250) {
			...
			setk(seld, "1");
			cm.sendOk("#fs14##r레벨업 달성보상이 지급되었습니다.");
			cm.dispose();
		} else {
			selr = reward[seld];
			if (getk(selr['lvl']) > -1) {
				cm.sendOk("#fs14##r이미 수령하신 보상입니다."+enter+enter+getRewardList());
				cm.dispose();
				return;
			}
			if (selr['lvl'] > cm.getPlayer().getLevel()) {
				cm.sendOk("#fs14##r이 보상을 받기엔 레벨이 모자랍니다."+enter+enter+getRewardList());
				cm.dispose();
				return;
			}
			...
			gainReward(selr['lvl']);
			setk(selr['lvl'], "1");
			cm.sendOk("#fs14##r보상 지급이 완료되었습니다.");
			cm.dispose();
		}
...
function getRewardList() {
	var msg = "#b"+selr['lvl']+"#fs14#레벨#k 달성 보상리스트 입니다.#fs14#"+enter;
	msg += ""+enter;
	for (p = 0; p < selr['item'].length; p++) {
		if (Math.floor(selr['item'][p]['itemid'] / 1000000) == 1) {
