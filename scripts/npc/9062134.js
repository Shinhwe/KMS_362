...
ccoin = 4310237; // 코인코드
    ...
    if (status == 0) {
		...
		if (st > cm.itemQuantity(ccoin) || st < 0) {
			cm.sendOk("#fn나눔고딕 Extrabold##fs14# #fc0xFF000000# 오류가 발생하였습니다.");
			cm.dispose();
			return;
		}
		말 = "#fs14##fn나눔고딕 Extrabold# #d 원하시는 몬스터를 선택해 주세요.\r\n\r\n"
		...
		h++;
		말 = "#fs14# #fc0xFF000000# 몬스터 레이스 "+h+"회차입니다.\r\n"
		말+= "#fs14# #fc0xFF000000# #r계속하시려면 엔터를 눌러주세요. ESC를 누를시 결과와 상관없이 코인이 지급되지 않습니다.#k#n\r\n\r\n";
		for (i=0; i<move1; i++) {
			말 += " ";
		}
		말+= "#fMob/"+dal[0]+"/move/0#\r\n"
		...
		if (st2 == win) {
			cm.sendOk("#fn나눔고딕 Extrabold##fs14##d 승리하셨습니다. 코인을 2배로 드릴게요!");
			cm.gainItem(ccoin, st*2);
		} else {
			cm.sendOk("#fs14##fn나눔고딕 Extrabold##d 패배하셨습니다.");
		}
		cm.dispose();
    }
}
