...
/* 스크립트 메인 부분 */
if (status == 0) {
	cm.sendSimple("#fs14##b"+cm.getPlayer().getName()+"#fc0xFF000000#님, #fc0xFF990033#[해피 스토리]#fc0xFF000000#에 오신 것을 환영합니다.\r\n#r(추천은 1인 1계정 레벨 200이상 가능합니다.)#k\r\n\r\n#fUI/UIWindow.img/UtilDlgEx/list1#\r\n#L0##b추천인#fc0xFF000000# 등록하기\r\n#L1##b추천인#fc0xFF000000# 랭킹보기#l\r\n\r\n\r\n#fUI/UIWindow.img/UtilDlgEx/list0#\r\n#L2##b추천인#fc0xFF000000# 확인하기\r\n");

} else if (status == 1) {
if (selection == 0) {
	if (!overlab_recom(cm.getClient().getAccID(), cm.getPlayer().getName())) {
		if (cm.getPlayer().getLevel() >= 200) {
			cm.sendGetText("#b#fs14#"+cm.getPlayer().getName()+"#fc0xFF000000#님, 당신을 #fc0xFF990033#[해피 스토리]#fc0xFF000000#로 이끈 이의 #b닉네임#fc0xFF000000#을 말씀해주세요.\r\n하지만 #r한 번 등록하면 되돌릴 수 없으니#fc0xFF000000# 신중하게 등록하셔야 해요.");
		} else {
			cm.sendOk("#fs14##r200레벨 미만#k은 추천인을 등록할 수 없어요.");
			cm.dispose();
		}
	} else {
		cm.sendOk("#fs14##fc0xFF000000#추천인은 한번만 작성가능합니다.");
		cm.dispose();
	}

} else if (selection == 1) {
	cm.sendSimple("#fs14##fc0xFF000000#이곳은 많은추천을 받은분들의 목록이에요.\r\n#b"+cm.getPlayer().getName()+"#fc0xFF000000#님께서도 조금만 노력한다면 이곳에 오르실 수 있어요.\r\n"+recom_log());
	status = 2;

} else if (selection == 2) {
	recom_num(cm.getPlayer().getName());
	if (recoms_num == 0) cm.sendOk("#fs14##fc0xFF000000#아직 #b"+cm.getPlayer().getName()+"#fc0xFF000000#님을 추천하신 분이 없네요.\r\n열심히 #fc0xFF990033#[해피 스토리]y#fc0xFF000000#를 알린다면, 보상이 따를 것 입니다."), cm.dispose();
	else {
	...
	var msg = "#fs14#받으실 보상을 선택해주세요.\r\n현재 #b#h ##k님의 추천 수는 개 입니다.#fs14#\r\n";
	...
	if (seld == 3) {
		seld2 = selection;
		var msg = "다음은 누적 "+reward[seld2]['goal']+"명 보상입니다.#b#fs14#\r\n";
		...
		if (cm.getText().equals("") || cm.getText().equals(cm.getPlayer().getName()) || getAccIdFromDB(cm.getText()) == getAccIdFromDB(cm.getPlayer().getName())) {
			...
			cm.sendOk("#fs14#이건 #b"+cm.getPlayer().getName()+"#fc0xFF000000#님에게 드리는 저의 작은 선물입니다. 앞으로의 여행에 큰 도움이 될 거예요.#b\r\n#i4001716##z4001716# 1개\r\n#i4310266##z4310266# 300개");
