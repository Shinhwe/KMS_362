...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        말 = "#fs14#" + 검정 + "#h #! 크크크 자네도 소식을듣고 여기까지 찾아온거군, 그렇다네! #r블랙 서버#k" + 검정 + "에선 신비한 힘이 담긴 특별한 스킬들이 있다네! \r\n";
        ...
        말 = "#fs14##s" + skillid + "# " + 노랑 + "" + skillname + "#k " + 검정 + "스킬 정보라네!\r\n"
        말 += "#fc0xFFD5D5D5#────────────────────────────#k\r\n";
        말 += "" + 검정 + "" + skilltesc + "\r\n\r\n"
        말 += "" + 검정 + "─ 스킬 효과 ─\r\n"
        ...
        if (selection == 100) {
            ...
            if (cm.getPlayer().getSkillLevel(skillid) <= 0) {
                cm.sendOkS("#fs14#" + 검정 + "자네는 " + 스킬이름 + " 스킬이 없는거 같군, 스킬을 구매해야 업그레이드가 가능하다네.", 0x04, 9401232);
                cm.dispose();
            } else {
                말 = "#fs14# " + 노랑 + "#s" + skillid + "# " + 스킬이름 + "#k" + 검정 + " 스킬을 정말 강화하겠나?\r\n";
                말 += "#fc0xFFD5D5D5#────────────────────────────#k\r\n";
            ...
            말 = "#fs14#" + 노랑 + "#s" + selection + "# " + sel + "#k 스킬" + 검정 + "을 정말로 구매하겠나?\r\n"
            말 += "#fc0xFFD5D5D5#────────────────────────────#k\r\n";
        ...
        if (구매 == 1) {
            if (cm.getPlayer().getSkillLevel(skillid) > 0) {
                cm.sendOkS("#fs14#" + 검정 + "이미 스킬을 보유하고 있는거 같네.", 0x04, 9401232);
                cm.dispose();
                return;
            }
            if (!cm.haveItem(4310012, 개수)) {
                cm.sendOkS("#fs14#" + 검정 + "구매에 필요한 코인이 부족한거 같네.", 0x04, 9401232);
                cm.dispose();
                return;
            }
            ...
            cm.sendOkS("#fs14#" + 검정 + "자네 스킬창에 " + 노랑 + "#s" + skillid + "#" + sel + "" + 검정 + "을(를) 지급했다네!", 0x04, 9401232);
            ...
            if (강화 == 0) {
                ...
                if (cm.getPlayer().getSkillLevel(skillid) >= level) {
                    cm.sendOkS("#fs14##s" + skillid + "# " + 노랑 + "" + 스킬이름 + " " + 검정 + "스킬은 이미 " + 레드 + "마스터레벨#k을 달성했네!", 0x04, 9401232);
                    cm.dispose();
                } else {
                    //if (cm.getClient().getQuestStatus(50001) == 1 || cm.getPlayer().getQuestStatus(50001) == 1) {
                        if (cm.getClient().getQuestStatus(50001) == 1) {
                    ...
                    if (!cm.haveItem(4310012, 강화갯수)) {
                        cm.sendOkS("#fs14#" + 검정 + "자네는 #z4310012# 아이템이 부족한거 같군.", 0x04, 9401232);
                        cm.dispose();
                        return;
                    }
                    cm.gainItem(4310012, -강화갯수);
                    cm.teachSkill(skillid, cm.getPlayer().getSkillLevel(skillid) + 1, level);
                    cm.sendOkS("#fs14##s" + skillid + "# " + 노랑 + "" + 스킬이름 + "#k " + 검정 + "업그레이드가 성공하여 #r" + (nextskillv) + "#k " + 검정 + "레벨이 되었다네!", 0x04, 9401232);
                    cm.dispose();
                }
            }
        }
