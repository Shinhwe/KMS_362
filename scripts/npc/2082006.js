...
function action(mode, type, sel) {
	...
	if (status == 0) {
		var msg = "#fs14##fc0xFF000000#으아앙~ 무지개 그림을 그려야하는데 크레파스가 없어서 못그리고있어.. 훌쩍\r\n"+enter;
		...
		temp.sort();
		max = temp[0];
		cm.sendGetNumber("#fs14##fc0xFF000000#최대 #b"+max+"개#fc0xFF000000# \r\n몇 번 바꿔줄까?", 1, 1, max);
	} else if (status == 2) {
		if (!haveNeed(sel)) {
		...
		cm.sendOk("#fs14##fc0xFF000000#고마워! 덕분에 그림을 그릴 수 있겠어!");
		cm.dispose();
	}
}
