...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			var ask = "#fs14#어느 타입의 아이템을 #r택배#k로 보내시겠습니까?\r\n택배를 보내기위해서는 #i5330000# 1개가 필요합니다.\r\n";
		...
		cm.dispose();
			...
			if (selection >= 1 && selection <=6) {
				cm.sendGetText("#fs14# 택배받으실 분의 아이디를 입력해주세요.#r\r\n\r\n택배시스템은 아이템의 종류나 옵션에 관계없이 누구에게나 택배할수 있는 시스템입니다. 택배를 보내기 위해선 퀵배송이용권이 필요하며, 같은 채널에 접속중이여야 합니다. 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않습니다, 고유 아이템 택배시 이미 아이템을 보유 중 일시 증발되며 이에 대한 복구는 책임지지 않습니다.");
			} else if (selection == 7) {
				cm.sendOk("#fs14# 택배시스템은 아이템의 종류나 옵션에 관계없이 누구에게나 택배할수 있는 시스템입니다. 택배을 하기 위해선 5억 메소가 필요하며, 같은 채널에 접속중이여야 합니다. 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않습니다.");
				cm.dispose();
			}
		} else if (status == 2) {
			if (operation == 1) {
		...
		if (conn == null){
		cm.sendOk("#fs14# 현재 접속중이 아니거나 채널이 다릅니다. 혹은 존재하지 않는 아이디일 수도 있습니다.");
		cm.dispose();
		}else{
		var ok = false;
		...
		if (!ok) {
			cm.sendOk("#fs14# 택배할 아이템이 없는것 같은데요?");
			cm.dispose();
			return;
		}
		...
		sel = selection;
			...
			selected = selection % 1000;
				...
				if (statsSel == null) {
					cm.sendOk("#fs14# 오류입니다. 운영자에게 보고해주세요.");
					cm.dispose();
					return;
				}
			var text = cm.getText();
		...
		if (cm.haveItem(5330000, 0)) {
			if (GameConstants.isPet(item.getItemId()) == false) {
				if (cm.getPlayer().getName() != text) {
					...
					//	cm.sendOk("#fs14# 택배을 받을 상대방의 인벤토리에 빈 공간이 없습니다.");
					//	cm.dispose();
					//	return;
					//}
					if (isban) {
			...
			cm.dispose();
				...
				cm.sendOk("#fs14# 자기 자신에게는 택배할수 없습니다.");
				cm.dispose();
			}
			}else {
				cm.sendOk("#fs14# 펫은 택배할수 없습니다.");
				cm.dispose();
			}
			}else{
