...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        txt = "#fs14#" + 검정 + "현재 자네가 보유하고 있는 #b치장 아이템#k" + 검정 + "이라네, #r모루#k" + 검정 + "하고 싶은 아이템을 선택해보게! 아참, 모루를 하기 위해선 #b3,000만 메소#k" + 검정 + "가 필요하다네 크크.\r\n#r#r마스터라벨 아이템과 클론 아바타만 모루가 가능하며, 무기는 모루 외형이 불가능합니다.#k#b#fs14#\r\n";
        ...
        if (a == 0) {
            cm.sendOkS("#fs14#자네는 인벤토리 창에 아이템이 없는거 같군, 마스터라벨 아이템과 클론 아바타만 이용이 가능하다네! 크크.", 0x04, 9401232);
            cm.dispose();
        } else {
        ...
        if (cm.getPlayer().getMeso() < 30000000) {
            cm.sendOkS("#fs14#" + 검정 + "모루를 하기 위한 비용 #r30,000,000 메소#k" + 검정 + "가 부족하다네.", 0x04, 9401232);
            cm.dispose();
            return;
        }
        선택한탬 = inz.getItem(selection);
        txt = "#fs14#" + 검정 + "모루할 아이템을 선택했군! 장비 아이템과 치장 아이템에서\r\n#r외형을 변경할 아이템#k" + 검정 + "을 선택해보게. 크크.\r\n"
        txt += "#fc0xFFD5D5D5#────────────────────────────#k#fs11\r\n";
        ...
        if (selection == 1) {
            ...
            if (check == 0) {
                cm.sendOkS("#fs14##r마스터라벨 아이템과 클론 아바타#k만 가능하다네.", 0x04, 9401232);
                cm.dispose();
                return;
            }
        }
        선택 = selection;
        txt = "#fs14#" + 보라색 + "선택한 아이템 [ #i" + 선택한탬.getItemId() + "# #t" + 선택한탬.getItemId() + "##l]#fs14##b\r\n외형으로 등록할 아이템을 선택해주세요.\r\n\r\n"
        if (선택 == 2) {
            inz = cm.getInventory(6)
            for (w = 0; w < inz.getSlotLimit(); w++) {
                if (!inz.getItem(w)) {
            ...
            txt += "#fs14##r장비 아이템을 모루 할 경우엔 다른 아이템으로 장착을 한 후 그 아이템을 장착 해제 해주시길 바랍니다.#b\r\n\r\n"
            for (w = 0; w < inz.getSlotLimit(); w++) {
                if (!inz.getItem(w)) {
                    continue;
                }
        ...
        if (모루탬.getMoru() > 0) {
            ...
            txt += "#fs14##b모루할 아이템 [ #i" + 선택한탬.getItemId() + "# #t" + 선택한탬.getItemId() + "##l]\r\n#r외형 아이템 [ #i" + itemid + "# #t" + itemid + "##l]\r\n\r\n"
        } else {
        ...
        cm.sendOk("#fs14#" + 검정 + "성공적으로 모루를 마쳤습니다.");
        cm.dispose();
    }
}
