...
function action(mode, type, sel) {
   ...
   if (status == 0) {
        ...
        switch (seld) {
            case 1:
                var msg ="#fs14##b#i4034939#30개, #i4034941#30개#k를 모아오면 특별한 선물을 줄게~#b"+enter+enter;
                    msg +="#i4034939#  #c4034939# / 30\r\n#i4034941#  #c4034941# / 30"+enter;
        
                cm.sendNext(msg);
                break;
            case 2:
                var msg = "#fs14#아래의 아이템을 모아오면 특별한 아이템을 줄게!#b"+enter+enter;
        ...
        switch (seld) {
            case 1:
                ...
                if (!pass) {
                    cm.sendOk("#fs14#아이템을 전부 모아오지 못한 것 같은데?");
                    cm.dispose();
                    return;
                }

                var msg = "#fs14#재료를 다 모아왔구나!?";
                cm.sendNext(msg);
                break;
            case 2:
                for (i = 0; i < special_needs.length; i++) {
                ...
                var msg = "#fs14#이번 이벤트의 특별한 아이템이야! 어때!?#b"+enter+enter;
        ...
        switch (seld) {
            case 1:
                var msg ="#fs14#모아온 재료들을 주면 아래 아이템 목록 중 하나를 랜덤으로 줄게 어때?#b"+enter+enter;
         ...
         if (reward[i][0] == -5)
            ...
            case 2:
      seld2 = sel;
                var msg = "#fs14#어때 정말 멋진 아이템이지? 이번 이벤트가 끝나면 얻을 수 없으니 운 좋은줄 알라구!#b"+enter+enter;
      if (sel == 1)
