...
function action(M, T, S) {
	if(M == -1) { cm.dispose(); } else {
		...
		if(Status == 0) {
		cm.sendSimple("#fs14#인벤토리에 있는 캐시 아이템을 버릴 수 있다. 버리고 싶은 아이템이 있는 인벤토리를 선택하자…"
			+ "\r\n#L0#캐시 장비 인벤토리"
//			+ "\r\n#L2#장비 인벤토리"
			+ "\r\n#L1#캐시 인벤토리");
		}
		...
		Say = "#fs14#버릴 아이템을 선택하자… 한 번 버린 아이템은 다시 되찾을 수 없다. 똑같은 것이 여러 개라면, 선택한 것을 버리게 된다.";
		S1 = S;
			switch(S1) {
			case 0: // 장비 인벤토리
		...
		S2 = S;
		cm.sendSimple("#fs14#선택한 아이템을 정말 처리할까?\r\n#b"
			+ "#L1#아이템을 삭제한다.\r\n\r\n\r\n#l"
			//+ "#L0#아이템을 버린다.\r\n\r\n\r\n#l"
