    ...
    if (status == 0) {
        ...
        if (selection == 0) { // 아이템 버리기
            ...
            if (cm.getPlayer().getInnerSkills().size() > 2) {
                cm.sendOk("#fs14#" + 검정 + "이미 어빌리티가 개방되어 있다네!#k", 9401232);
                cm.dispose();
                return;
            }
            ...
            cm.fakeRelog();
            cm.updateChar();
            cm.sendOk("#fs14#" + 검정 + "자네를 위해 어빌리티를 개방해주었다네!", 9401232);
            cm.dispose();
        } else if (selection == 5) { // 포켓
            cm.getPlayer().forceCompleteQuest(6500);
            cm.sendOk("#fs14#" + 검정 + "자네를 위해 포켓을 개방해주었다네!", 9401232);
            cm.dispose();
        } else if (selection == 6) { // 펫 교환
            말 = "#fs14#" + 검정 + "교환하고 싶은 라벨의 펫을 선택해보게.\r\n"
            말 += "#fc0xFFD5D5D5#───────────────────────────#k\r\n";
            말 += "#fs14##L0#" + 펫 + "#fc0xFF6B66FF# 라벨 펫#k" + 검정 + "을 교환하고 싶습니다.#l\r\n"
            말 += "#fs14##L1#" + 펫1 + "#fc0xFFA566FF# 라벨 펫#k" + 검정 + "을 교환하고 싶습니다.#l\r\n"
            ...
            cm.sendOkS("#fs14##b원데이 샵#k을 이용하기 위해선 #r200#k레벨이 되어야 합니다!", 2)
            cm.dispose();
        }
